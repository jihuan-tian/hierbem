...
// @file hmat.asy Visualization of H-matrices using Asymptote.
// @author Jihuan Tian
// @date 2023-12-12
...
void plot_bct_struct(picture pic = currentpicture, string filename, pen near_field_fillpen = rgb(2.6700e-01, 4.8743e-03, 3.2942e-01), pen far_field_fillpen = rgb(0.1306, 0.5577, 0.5522), pen border_pen = rgb(0.9932, 0.9062, 0.1439))
{
  ...
  while(!eof(fin))
      ...
      if (c == '[')
	{
	  ...
	  if (is_near_field == 1)
	    {
	      plot_matrix_block(pic, (tau[0], tau[1]), (sigma[0], sigma[1]), near_field_fillpen, border_pen);
	    }
	  else
	    {
	      plot_matrix_block(pic, (tau[0], tau[1]), (sigma[0], sigma[1]), far_field_fillpen, border_pen);
	    }
	}
      else
	{
