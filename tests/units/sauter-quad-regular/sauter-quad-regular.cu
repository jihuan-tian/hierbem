...
{
  /**
   * Generate a mesh containing six cells, where cells 0 and 5 as well as 1 and
   * 4 are separated.
   */
...

  {
    ...
    /**
     * Generate finite element, which is shared by both test and ansatz
     * spaces.
     */
     ...
     * neighboring types. Even though only the regular case is considered in
    ...
    bem_values.shape_function_values_regular();
...

  {
    ...
    /**
     * Generate 4D Gauss-Legendre quadrature rules for various cell
     * neighboring types. Even though only the same panel case is considered
     * in this testcase,
          ...
          for (unsigned int j = 0; j < slp_cell_matrix.n(); j++)
            {
              slp_cell_matrix(i, j) =
                sauter_quadrature_on_one_pair_of_shape_functions(
                  ...
                  i,
                  j,
                  cell_iterators_neumann_space[0],
                  cell_iterators_neumann_space[5],
          ...
          for (unsigned int j = 0; j < dlp_cell_matrix.n(); j++)
            {
              dlp_cell_matrix(i, j) =
                sauter_quadrature_on_one_pair_of_shape_functions(
                  ...
                  j,
                  cell_iterators_neumann_space[0],
                  cell_iterators_dirichlet_space[5],
                  mapping_info_test_space,
                  mapping_info_ansatz_space,
                  bem_values,
                  OutwardSurfaceNormalDetector(),
          ...
          for (unsigned int j = 0; j < adlp_cell_matrix.n(); j++)
            {
              adlp_cell_matrix(i, j) =
                sauter_quadrature_on_one_pair_of_shape_functions(
                  ...
                  i,
                  j,
                  cell_iterators_dirichlet_space[0],
                  cell_iterators_neumann_space[5],
                  mapping_info_test_space,
                  mapping_info_ansatz_space,
                  bem_values,
