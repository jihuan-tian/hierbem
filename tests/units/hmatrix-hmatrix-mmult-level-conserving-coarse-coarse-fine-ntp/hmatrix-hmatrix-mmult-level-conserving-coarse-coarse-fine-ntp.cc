/**
 * \file hmatrix-hmatrix-mmult-level-conserving-coarse-coarse-fine-ntp.cc
 * \brief Verify the multiplication of two level-conserving
 * \f$\mathcal{H}\f$-matrices. Both operands have the coarse non-tensor product
 * partitions, while the result matrix has the fine non-tensor product
 * partitions.
 *
