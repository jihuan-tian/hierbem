...
{
  std::ofstream ofs(
    "hmatrix-cholesky-transpose-forward-substitution-matrix-valued.output");
   ...
   * Here we enforce each far field matrix block to have a full rank, i.e. its
   * rank is the same as that of the corresponding full matrix block. In this
   * way, the constructed H-matrix can accurately represent the original full
   * matrix, which produces a very small relative error in the following
   * computation and make catch2 tests pass.
   *
   ...
   * substitution.
   */
  HL.solve_cholesky_transpose_by_forward_substitution_matrix_valued(HX,
                                                                    HZ,
                                                                    fixed_rank);
...
run_hmatrix_cholesky_transpose_forwardsubst_matrix_valued_in_situ()
{
   ...
   * substitution.
   */
  HL.solve_cholesky_transpose_by_forward_substitution_matrix_valued(HZ,
                                                                    fixed_rank);
