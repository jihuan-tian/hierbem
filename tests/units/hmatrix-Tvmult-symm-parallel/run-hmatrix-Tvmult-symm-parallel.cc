...
void
run_hmatrix_Tvmult_symm_parallel()
{
  std::ofstream ofs("hmatrix-Tvmult-symm-parallel.output");
   ...
   * @p vmult_task_parallel will be called by @p Tvmult_task_parallel internally.
   * Both vmult and Tvmult thread data are needed in the preparation.
   */
  H.prepare_for_vmult_or_tvmult(true, true);
  H.Tvmult_task_parallel(0.3, y, 1.5, x);
  print_vector_to_mat(ofs, "y1_cpp", y);

  H_complex.prepare_for_vmult_or_tvmult(true, true);
  H_complex.Tvmult_task_parallel(0.3, y_complex, 1.5, x_complex);
  print_vector_to_mat(ofs, "y1_cpp_complex", y_complex);
  H_complex.Tvmult_task_parallel(std::complex<double>(0.3, 0.2),
                                 y_complex,
  ...
  H_complex.Tvmult_task_parallel(0.3,
                                 y_complex,
                                 std::complex<double>(1.5, 2.1),
                                 x_complex);
