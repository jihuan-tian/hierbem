...
//  gridout.write_msh(triangulation, mesh_file);
  ...
  {
    std::vector<Point<spacedim> > real_support_points(HierBEM::support_points_in_real_cell(cell, fe, mapping, fe.get_poly_space_numbering_inverse()));

    cell_surface_jacobian_det(i) =
        HierBEM::surface_jacobian_det(fe, real_support_points, unit_cell_center);
