...
main()
{
  TreeNode<int, 4> *tree_orig = MakeIntExampleTree();
  TreeNode<int, 4> *tree_copy = CopyTree(tree_orig);

  std::cout << "Original tree:\n";
  PrintTree(std::cout, tree_orig);
  DeleteTree(tree_orig);

  std::cout << "Copied tree:\n";
  PrintTree(std::cout, tree_copy);
