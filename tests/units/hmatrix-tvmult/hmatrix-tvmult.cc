 ...
 * \brief Verify \hmatrix transposed matrix-vector multiplication.
 * \ingroup testers
 * \author Jihuan Tian
 * \date 2022-11-28
 */
...
main()
{
  ...
  print_vector_to_mat(std::cout, "y_trans1", y_trans);

  y_trans = 0.;
  hmat.Tvmult(y_trans, 0.5, x);
  print_vector_to_mat(std::cout, "y_trans2", y_trans);
}
