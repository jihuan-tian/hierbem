  ...
  const IterativeSolverVmultType vmult_type)
{
  ...
  HelmholtzAcousticBEM<dim, spacedim> bem(
    ...
    HMatrixParameters<double>(4, 4, 0.8, 5, 0.01), // H-matrix parameters
    HMatrixParameters<double>(
      4, 4, 1.0, 2, 0.1), // H-matrix preconditioner parameters, when the
                          // preconditioner type is @p HMatrixFactorization.
