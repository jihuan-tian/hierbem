...
extern void
run_dirichlet_full_matrix(const bool is_serial);

TEST_CASE(
  "Solve Laplace problem with Dirichlet boundary condition using full matrix",
  "[laplace]")
{
  ...
  SECTION("serial")
  {
    run_dirichlet_full_matrix(true);
  ...
  SECTION("parallel")
  {
    run_dirichlet_full_matrix(false);
