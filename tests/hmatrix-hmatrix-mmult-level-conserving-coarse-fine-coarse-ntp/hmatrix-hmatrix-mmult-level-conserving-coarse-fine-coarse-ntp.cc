/**
 * \file hmatrix-hmatrix-mmult-level-conserving-coarse-fine-coarse-ntp.cc
 * \brief Verify the multiplication of two level-conserving
 * \f$\mathcal{H}\f$-matrices. The first operand and the result matrix have the
 * coarse non-tensor product partitions, while the second operand has the fine
 * non-tensor product partitions.
 *
