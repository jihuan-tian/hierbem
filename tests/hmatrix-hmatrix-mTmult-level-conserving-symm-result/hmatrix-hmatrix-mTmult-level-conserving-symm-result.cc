/**
 * \file hmatrix-hmatrix-mmult-level-conserving-symm-result.cc
 * \brief Verify the \hmat multiplication \f$M \cdot M^T\f$, hence the result
 * matrix is symmetric.
 *
...
main()
{
   ...
   */
  LAPACKFullMatrixExt<double> H1_full, H1_mult_H1T_full;
  H1.convertToFullMatrix(H1_full);
  H1_full.print_formatted_to_mat(std::cout, "H1_full", 16, false, 25, "0");

  H1_full.mTmult(H1_mult_H1T_full, H1_full);
  H1_mult_H1T_full.print_formatted_to_mat(
    std::cout, "H1_mult_H1T_full", 16, false, 25, "0");

  /**
   * Multiply the \hmatrices
   */
  H1.mTmult_level_conserving(H, H1, fixed_rank_k, true);
