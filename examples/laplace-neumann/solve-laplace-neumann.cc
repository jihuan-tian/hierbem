...
public:
  ...
  NeumannBC(const Point<3> &x0_, const Point<3> &center_, double radius_)
    : Function<3>()
    , x0(x0_)
    , center(center_)
    , radius(radius_)
...
main()
{
  ...
  ConfHMatrix hmat_params{32, 32, 8, 4, 0.8, 10, 10, 0.01, false, 10};
  ConfHMatrix hmat_preconditioner_params{
    32, 32, 8, 4, 1.0, 5, 5, 0.1, false, 10};
  ...
  LaplaceBEM<dim, spacedim, double, double> bem(bem_params,
                                                ...
                                                op_precond_params,
                                                parallel_params);
  bem.set_project_name("solve-laplace-neumann");
  ...
  output_analytical_solution(bem, source_loc);
}
