...
\begin{Theorem}[Implicit function theorem]
  ...
  $\abs{\frac{\partial f}{\partial y}}_{x=a, y=b} \neq 0$. Then $\exists$ neighborhood $B$ of
...
\begin{Proof}
  ...
  $$
  Therefore, $\abs{\frac{\partial f'}{\partial y}}_{x=a, y=b} \neq 0$. According to Theorem
...
\begin{Corollary}
  ...
  $\rank\left( \left[ \frac{\partial f}{\partial x} \right] \bigg\vert_{x=x_0} \right)=r$,
...
\begin{Example}[Sphere ($S^2$) as a two dimensional submanifold in $\mathbb{R}^3$]
  ...
  \begin{equation*}
    \frac{\partial z}{\partial x} = \frac{-x}{\sqrt{1 - x^2 - y^2}}, \; \frac{\partial z}{\partial
      y} = \frac{-y}{\sqrt{1 - x^2 - y^2}}.
  \end{equation*}
  When $(x,y,z)$ approaches to the equator, $\frac{\partial z}{\partial x}$ and
  $\frac{\partial z}{\partial y}$ will be infinite. Therefore, the upper hemisphere excluding
...
\begin{Definition}[Submanifold of manifold]
  ...
  $$
  whose Jacobian matrix $\frac{\partial F}{\partial x}$ has rank $n-r$ at each point of U.
...
\begin{Definition}[Tangent vector]
  ...
  \begin{equation}
    ...
    v_V^i = \sum_{j=1}^n \frac{\partial x_V^i}{\partial x_U^j} \Bigg\vert_p v_U^j.
  \end{equation}
\end{Definition}
...
\begin{equation}
  ...
  v_V = \left[ \frac{\partial x_{V}}{\partial x_U} \right] \Bigg\vert_p v_U,
\end{equation}
where $\left[ \frac{\partial x_{V}}{\partial x_U} \right]\Big\vert_p$ is the Jacobian matrix of the transition
...
\begin{equation}
  \begin{pmatrix}
    \frac{\partial x_V^1}{\partial x_U^1} & \cdots & \frac{\partial x_V^1}{\partial x_U^n} \\
    \vdots & \vdots & \vdots \\
    \frac{\partial x_V^n}{\partial x_U^1} & \cdots & \frac{\partial x_V^n}{\partial x_U^n}
  \end{pmatrix}.
\end{equation}
...
\begin{equation}
  \label{eq:df-in-xu}
  D_{\vect{v}_U}(f) = \sum_{i=1}^n \frac{\partial f}{\partial x_U^i} \frac{\diff
    x_{U}^i(t)}{\diff t} = \sum_{i=1}^n \frac{\partial f}{\partial x_U^i}\Bigg\vert_{x_U=p_U} v_U^i.
\end{equation}
If another coordinate chart $(V, x_V)$ is adopted which overlaps with $(U,x_U)$, the
derivative of $f$ can also be written as
\begin{equation}
  \label{eq:df-in-xv}
  D_{\vect{v}_V}(f) = \sum_{i=1}^n \frac{\partial f}{\partial x_V^i} \frac{\diff
    x_{V}^i(t)}{\diff t} = \sum_{i=1}^n \frac{\partial f}{\partial x_V^i} \Bigg\vert_{x_V=p_V} v_V^i.
\end{equation}
...
\begin{Proof}[Directional derivative of $f$ is coordinate independent]
  ...
  \begin{equation}
    ...
    v_V^i = \sum_{j=1}^n \frac{\partial x_{V}^i}{\partial x_{U}^j} v_U^j \quad (i=1,\cdots,n).
  \end{equation}
  Substitute this transformation into Equation (\ref{eq:df-in-xv}), we have
  $$
  D_{\vect{v}_V}(f) = \sum_{i=1}^n \frac{\partial f}{\partial x_V^i} \sum_{j=1}^n \frac{\partial
    x_{V}^i}{\partial x_{U}^j} x_U^j = \sum_{j=1}^n \left( \sum_{i=1}^n \frac{\partial f}{\partial
      x_V^i} \frac{\partial x_{V}^i}{\partial x_{U}^j} \right) x_U^j = \sum_{j=1}^n \frac{\partial
    f}{\partial x_U^j} x_U^j = D_{\vect{v}_U}(f).
  $$
...
\begin{equation}
  ...
  v=\sum_j v^j \frac{\partial}{\partial x^j}\Bigg\vert_p.
\end{equation}
Here $v^j$ is the vector coefficient and the differential operator
$\frac{\partial }{\partial x^j}\Big\vert_p$ is the corresponding basis vector, which will be
written as $\frac{\vect{\partial}}{\vect{\partial} x^j}$ in bold typeface from now on or in a
simpler form $\vect{\partial} x^j$. Hence, $\left\{ \vect{\partial} x^j \right\}_{j=1}^n$ is a
basis of the coordinate chart $(U,x)$. Since the $j$-th component of $v$ in this chart is
just $v^j$, the corresponding basis vector $\vect{\partial} x^j$ is numerically the same as
the basis vector $\boldsymbol{e}_j$ of the Cartesian frame $\mathbb{R}^n$, i.e. the $i$-th
component of $\vect{\partial}x^j$ is the Kronecker $\delta_j^i$. However, it is not a global
...
axis $\vect{\partial}x^j$ is not necessarily 1, since $\vect{\partial}x^j$ may not be a
...
\end{equation}
This is just equal to $\vect{\partial}x^j$ which is represented in the coordinate chart
$(U,x)$. Then the set of velocity vectors of all coordinate curves at $p$ using the
coordinate chart $(U,x)$ forms the basis of the tangent space at $p$, i.e.
$\left\{ \vect{\partial} x^j \right\}_{j=1}^n$. This is the same as the results derived
from treating a tangent vector as a differential operator. Numerically, a basis vector
$\boldsymbol{\partial}x^j$ represented in its respective coordinate chart $(U,x)$ is equal
to the $j$-th basis vector $\boldsymbol{e}_j$ of the Cartesian space $\mathbb{R}^d$. This
means the tangent space is similar to the Cartesian space. However, such similarity is
only locally valid, i.e. the basis is only applicable to an infinitesimal neighborhood
around $p$. This is because the coordinate curves may not be straight but curved. If the
point $p$ moves a little to a new point $p'$, the basis of the new tangent space rooted at
$p'$ may change their directions a little bit compared to the basis at $p$.
...
$\left\{ \vect{\partial}x^j \right\}_{j=1}^n$ of $(U,x)$ can be represented using the
coordinate variables of the new chart via the Jacobian matrix of the transition map
$\phi_{VU}$, since each basis vector is also a tangent vector, which should follow the
same transformation rule as in (\ref{eq:tangent-vector-transform}). For the basis vector
$\vect{\partial}x^j$, the $i$-th component in the transformed vector is
\begin{equation}
  \label{eq:tangent-space-basis-transform}
  \sum_{k=1}^n \frac{\partial y^i}{\partial x^k} (\vect{\partial} x^j)_k = \sum_{k=1}^n
  \frac{\partial y^i}{\partial x^k} \delta_k^j = \frac{\partial y^{i}}{\partial x^{j}} \quad (i=1,\cdots,n').
\end{equation}
This is just the $j$-th column of the Jacobian matrix
$$
\begin{pmatrix}
  \frac{\partial y^{1}}{\partial x^j} \\
  \vdots \\
  \frac{\partial y^{n'}}{\partial x^j}
\end{pmatrix}.
$$
...
\begin{Example}[Basis of tangent space for a sphere parameterized by $(\theta, \phi)$]
  ...
  $(\theta, \phi)$ is $\left\{ \vect{\partial}\theta, \vect{\partial}\phi \right\}$. In the
  chart $(x,y,z)$, the basis is
  \begin{equation}
    \begin{aligned}
      \vect{\partial}\theta &= r\cos\phi \cos\theta \vect{\partial}x + r\sin\phi \cos\theta
      \vect{\partial}y - r\sin\theta \vect{\partial}z \\
      \vect{\partial}\phi &= -r\sin\phi \sin\theta \vect{\partial}x + r\cos\phi \sin\theta
      \vect{\partial}y
    \end{aligned}.
  \end{equation}
  ...
  \end{figure}
  We also notice that the basis vectors $\vect{\partial}\theta$ and $\vect{\partial}\phi$ are
  not normalized:
  \begin{equation*}
    \begin{aligned}
      \norm{\vect{\partial}\theta} &= \sqrt{r^2\cos^2\phi\cos^{2}\theta +
        r^2\sin^2\phi\cos^2\theta + r^2\sin^2\theta} = \sqrt{r^2\cos^2\theta +
        r^2\sin^2\theta} = r \\
      \norm{\vect{\partial}\phi} &= \sqrt{r^2\sin^2\phi\sin^2\theta +
        r^2\cos^2\phi\sin^2\theta} = \sqrt{r^2\sin^2\theta} = r\sin\theta
    \end{aligned}.
  \end{equation*}
  Therefore, a normalized basis
  $\left\{ \vect{\partial}\hat{\theta}, \vect{\partial}\hat{\phi} \right\}$ of the spherical
  coordinate chart is
  $\left\{ \frac{1}{r} \vect{\partial}\theta, \frac{1}{r\sin\theta} \vect{\partial}\phi
  \right\}$.
\end{Example}
...
\begin{Example}[Basis of tangent space for a ball parameterized by $(r, \theta, \phi)$
  ...
  \begin{equation}
    \begin{aligned}
      \vect{\partial}r &= \cos\phi \sin\theta \vect{\partial}x + \sin\phi \sin\theta
      \vect{\partial}y + \cos\theta \vect{\partial}z \\
      \vect{\partial}\theta &= r\cos\phi\cos\theta \vect{\partial}x + r\sin\phi\cos\theta
      \vect{\partial}y - r\sin\theta \vect{\partial}z \\
      \vect{\partial}\phi &= -r\sin\phi \sin\theta \vect{\partial}x + r\cos\phi \sin\theta
      \vect{\partial}y
  ...
  $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\theta}, \vect{\partial}\hat{\phi}
  \right\}$ is
  $\left\{ \vect{\partial}r, \frac{1}{r} \vect{\partial}\theta, \frac{1}{r\sin\theta}
    \vect{\partial}\phi \right\}$.
\end{Example}
...
\begin{Example}[Basis of tangent space for a cylinder parameterized by $(r, \varphi, z')$]
  ...
  \begin{equation}
    \begin{aligned}
      \vect{\partial}r &= \cos\phi \vect{\partial}x + \sin\phi \vect{\partial}y \\
      \vect{\partial}\phi &= -r \sin\phi \vect{\partial}x + r \cos\phi \vect{\partial}y \\
      \vect{\partial}z' &= \vect{\partial}z
    \end{aligned}.
  \end{equation}
  The normalized basis is
  $\left\{ \vect{\partial}r, \frac{1}{r}\vect{\partial}\phi, \vect{\partial}z' \right\}$.
\end{Example}
...
space $M_p^n$. The basis vectors of the coordinate chart $(U,x_U)$ are
$\left\{ \vect{\partial}x_U^j \right\}_{j=1}^n$ and the basis vectors of $(V,x_V)$ are
$\left\{ \vect{\partial}x_V^i \right\}_{i=1}^{n'}$. Still as before, the number of
coordinate components in $x_U$ and $x_V$ may not be the same, i.e. $n\neq n'$. Then we
want to know how each basis vector $\vect{\partial}x_V^i$ in $(V,x_V)$ is numerically
represented using the basis $\left\{ \vect{\partial}x_U^j \right\}$ in $(U,x_U)$, i.e. we
want to compute the expansion coefficients $a_j$ in the following expression
\begin{equation*}
  \left( \vect{\partial}x_V^i \right)_{U} = \sum_j a^j \vect{\partial}x_U^j.
\end{equation*}
\begin{Remark}
  ...
  of different coordinate charts like $\vect{\partial}x_U^j$ and $\vect{\partial}x_V^i$ are
  two different physical entities, i.e. they are usually two different tangent vectors in
  $M_p^n$.
\end{Remark}

Because $\vect{\partial}x_V^i$ itself is a tangent vector, its transformation from $(V,x_V)$
into $(U,x_U)$ follows the rule of contravariant transformation in Equation
(\ref{eq:contravariant-transformation}):
\begin{equation*}
  \frac{\partial x_U}{\partial x_V} (\vect{\partial}x_V^i)^{\mathrm{T}}.
\end{equation*}
Note here we've transposed the basis vector $\vect{\partial}x_V^i$, because as a
convention, a tangent vector in the matrix form is written as a column vector, while the
matrix form of a basis vector in the tangent space is a row vector. Since
$\vect{\partial}x_V^i$ represented in $(V,x_V)$ is numerically the same as $\vect{e}_i$,
the above expression extracts the $i$-th column of the Jacobian matrix of $\phi_{UV}$.
Therefore, the expansion coefficient $a^{j}$ is just the $j$-th component in this column
vector, i.e.
\begin{equation}
  a^j = \frac{\partial x_U^j}{\partial x_V^i}.
\end{equation}
Hence, the basis vector $\vect{\partial}x_V^i$ can be numerically represented in $(U,x_U)$ as
\begin{equation}
  \label{eq:tangent-vector-basis-relation}
  \vect{\partial}x_V^i = \sum_j \frac{\partial x_U^j}{\partial x_V^i} \vect{\partial}x_U^j.
\end{equation}
If we horizontally juxtapose the basis vectors as
$(\vect{\partial}x_V^1,\cdots,\vect{\partial}x_V^n)$ and
$(\vect{\partial}x_U^1,\cdots,\vect{\partial}x_U^n)$, the matrix form of the relationship
between the two bases is
\begin{equation}
  \label{eq:tangent-space-bases-relation}
  (\vect{\partial}x_V^1,\cdots,\vect{\partial}x_V^n) =
  (\vect{\partial}x_U^1,\cdots,\vect{\partial}x_U^n) \left[ \frac{\partial x_U}{\partial x_V} \right].
\end{equation}
...
Apply $\alpha$ to the $i$-th basis vector $\vect{\partial}x^i$ of the tangent space, we
obtain the $i$-th expansion coefficient of $\alpha$:
\begin{equation}
  \label{eq:apply-1-form-to-basis}
  \alpha(\vect{\partial}x^i) = \sum_j a_j \sigma^j(\vect{\partial}x^i) = \sum_j a_j
  \delta^j_i = a_i.
\end{equation}
...
tangent space $M_p^n$ is $\left\{ \vect{\partial}x^j \right\}$ and the basis of the
...
\begin{equation}
  ...
  df(v) = D_v(f) = v(f) = df \left( \sum_j v^j \vect{\partial}x^j \right) = \sum_j v^j
  df(\vect{\partial}x^j) = \sum_j v^j \frac{\partial f}{\partial x^j}\Bigg\vert_p.
\end{equation}
...
If $f=x^i$, we have
\begin{equation*}
  df(v) = dx^i(v) = \sum_j v^j \frac{\partial x^{i}}{\partial x^j} = \sum_j v^j \delta^i_j = v^i.
\end{equation*}
...
$a_j = df(\vect{\partial}x^j) = \frac{\partial f}{\partial x^j} \Big\vert_p$. Hence,
\begin{equation}
  \label{eq:differential-of-0-form}
  df = \sum_j \frac{\partial f}{\partial x^j}\Bigg\vert_p dx^j.
\end{equation}
...
\begin{equation}
  ...
  dx_V^i=\sum_j \frac{\partial x_V^i}{\partial x_U^j} dx_U^j.
\end{equation}
Its matrix form is
\begin{equation}
  dx_V = \left[ \frac{\partial x_V}{\partial x_U} \right]\Bigg\vert_p dx_U,
\end{equation}
...
\begin{equation*}
  \alpha = \sum_i a_i^V \sum_j \frac{\partial x_V^i}{\partial x_U^j} dx_U^j = \sum_i\sum_j
  a_i^V \left( \frac{\partial x_V^i}{\partial x_U^j} \right) dx_U^j = \sum_j \left[ \sum_i
    a_i^V \left( \frac{\partial x_V^i}{\partial x_U^j} \right) \right] dx_U^j = \sum_j a_j^U dx_U^j,
\end{equation*}
where the expansion coefficient is
\begin{equation*}
  \label{eq:covector-transformation}
  a_j^U = \sum_i a_i^V \left( \frac{\partial x_V^i}{\partial x_U^j} \right).
\end{equation*}
Its matrix form is
\begin{equation}
  a^U = a^V \left[ \frac{\partial x_V}{\partial x_U} \right],
\end{equation}
where $a^U = (a_1^U,\cdots,a_n^U)$ and $a^V = (a_1^V,\cdots,a_n^V)$,
$\left[ \frac{\partial x_V}{\partial x_U} \right]$ is the Jacobian matrix of the transition map
$\phi_{VU}$. Similarly,
\begin{equation}
a^V = a^U \left[ \frac{\partial x_U}{\partial x_V} \right].
\end{equation}
...
We've already met two Jacobian matrices $\left[ \frac{\partial x_V}{\partial x_U} \right]$ and
$\left[ \frac{\partial x_U}{\partial x_V} \right]$. The former is the Jacobian matrix of the
...
components, $\left[ \frac{\partial x_V}{\partial x_U} \right] \in \mathbb{R}^{n\times m}$ and
$\left[ \frac{\partial x_U}{\partial x_V} \right]\in \mathbb{R}^{m\times n}$, i.e. both of
which may be rectangular matrices and have no inverse matrices.

The product of these two matrices are
\begin{equation}
  \left[ \frac{\partial x_V}{\partial x_U} \right] \cdot \left[ \frac{\partial x_U}{\partial x_V}
  \right] = \left\{ \sum_k \frac{\partial x_V^i}{\partial x_U^k} \frac{\partial x_U^k}{\partial
      x_V^j} \right\}_{ij} = \frac{\partial x_V^i}{\partial x_V^j} = \left\{ \delta^i_j
  \right\}_{ij} = I_n.
\end{equation}
Similarly,
\begin{equation}
  \left[ \frac{\partial x_U}{\partial x_V} \right] \cdot \left[ \frac{\partial x_V}{\partial x_U} \right] = I_m.
\end{equation}
...
$\left\{ \vect{\partial}x^i \right\}_{i=1}^n$. The basis of $M_p^{n*}$ is
$\left\{ dx^i \right\}_{i=1}^n$. Superscripts are adopted for
\begin{enumerate}
\item a component of the tangent vector $v$: $v^i$
\item index of a basis vector in the tangent space: $\vect{\partial}x^i$. \emph{N.B. The
    superscript $i$ used here is only formal. In fact, $\vect{\partial}x^i$ is short for
    $\frac{\partial}{\partial x^i}$. The superscript $i$ assigned to $x$ appears in the
...
cotangent vector $\alpha$ and $\vect{\partial}x_U^i$ is the $i$-th basis vector in this
chart.
...
\item a basis vector $\vect{\partial}x^i$ of the tangent space
\end{itemize}
...
\begin{equation}
  ...
  \left\langle a,b \right\rangle = \left\langle \sum_i a^i \vect{\partial}x^i, \sum_j b^j
    \vect{\partial}x^j \right\rangle = \sum_i\sum_j a^i b^j \left\langle \vect{\partial}x^i,
    \vect{\partial}x^j \right\rangle.
\end{equation}

\begin{Definition}[Metric tensor]
  \label{def:metric-tensor}
  $g_{ij} \coloneqq \left\langle \vect{\partial}x^i, \vect{\partial}x^j \right\rangle$ is
...
\begin{equation*}
  g_{ij}^V = \left\langle \vect{\partial}x_V^i, \vect{\partial}x_V^j \right\rangle
\end{equation*}
using the chart $(V,x_V)$. Or it can be represented as
\begin{equation*}
  g_{rs}^U = \left\langle \vect{\partial}x_U^r, \vect{\partial}x_U^s \right\rangle
\end{equation*}
in the chart $(U,x_U)$. According to Equation (\ref{eq:tangent-vector-basis-relation}), we
have
\begin{equation*}
  \vect{\partial}x_V^i = \sum_r \frac{\partial x_U^r}{\partial x_V^i} \vect{\partial}x_U^r
\end{equation*}
and
\begin{equation*}
  \vect{\partial}x_V^j = \sum_s \frac{\partial x_U^s}{\partial x_V^j} \vect{\partial}x_U^s.
\end{equation*}
Then
\begin{equation}
  \label{eq:metric-tensor-transformation}
  g_{ij}^V = \left\langle \sum_r \frac{\partial x_U^r}{\partial x_V^i} \vect{\partial}x_U^r,
    \sum_s \frac{\partial x_U^s}{\partial x_V^j} \vect{\partial}x_U^s \right\rangle =
  \sum_r\sum_s \frac{\partial x_U^r}{\partial x_V^i} \frac{\partial x_U^s}{\partial x_V^j}
  \left\langle \vect{\partial}x_U^r, \vect{\partial}x_U^s \right\rangle = \sum_r\sum_s
  \frac{\partial x_U^r}{\partial x_V^i} \frac{\partial x_U^s}{\partial x_V^j} g_{rs}^U.
\end{equation}
Its matrix form is
\begin{equation}
  \label{eq:metric-tensor-transformation-matrix-form}
  G^V = \left[ \frac{\partial x_U}{\partial x_V} \right]^{\mathrm{T}} G^U \left[ \frac{\partial
      x_U}{\partial x_V} \right] = J_{UV}^{\mathrm{T}} G^U J_{UV}.
\end{equation}
...
\begin{Example}[Metric tensor of spherical coordinate chart]
  ...
  is $\left\{ \vect{\partial}x, \vect{\partial}y, \vect{\partial}z \right\}$ and the basis of
  $(V,x_V)$ is $\left\{ \vect{\partial}\theta, \vect{\partial}\phi \right\}$. The metric
  ...
  When $r=1$, the basis vector $\vect{\partial}\theta$ has unit length but
  $\vect{\partial}\phi$ usually does not. $\vect{\partial}\phi$ only has unit length when
...
\begin{Example}[Metric tensor of ball coordinate chart]
  ...
  $\left\{ \vect{\partial}r, \vect{\partial}\theta, \vect{\partial}\phi \right\}$, i.e. the
...
\begin{Example}[Metric tensor of cylindrical coordinate chart]
  ...
  $\left\{ \vect{\partial}x, \vect{\partial}y, \vect{\partial}z \right\}$ and the basis of
  $(V,x_V)$ is $\left\{ \vect{\partial}r, \vect{\partial}\phi, \vect{\partial}z \right\}$. Using
...
\begin{equation*}
  \alpha = \sum_j \alpha_j dx^j = \sum_j \alpha(\vect{\partial}x^j) dx^j = \sum_j
  \left\langle a, \vect{\partial}x^j \right\rangle dx^j.
\end{equation*}
If the tangent vector $a$ is expanded as $a = \sum_i a^i \vect{\partial}x^i$, we have
\begin{equation*}
  \alpha = \sum_j \left\langle \sum_i a^i \vect{\partial}x^i, \vect{\partial}x^j \right\rangle
  dx^j = \sum_j\sum_i a^i \left\langle \vect{\partial}x^i, \vect{\partial}x^j \right\rangle
...
Compare
\begin{equation*}
  df(v) = \sum_j \frac{\partial f}{\partial x^j} v^j
\end{equation*}
and
\begin{equation*}
...
\begin{equation}
  ...
  \left( \nabla f \right)^i = \sum_j g^{ij} \frac{\partial f}{\partial x^{i}}
\end{equation}
...
\begin{Example}[Gradient vector on sphere manifold $S^2$]
  ...
  $\left\{ \vect{\partial}\phi, \vect{\partial}\theta \right\}$ is adopted for the tangent space, the matrix form of
  $\nabla f$ is
  \begin{equation}
  ...
  \begin{equation}
    \nabla f = \frac{1}{r^2} \frac{\partial f}{\partial \theta} \vect{\partial}\theta +
    \frac{1}{r^2\sin^2\theta} \frac{\partial f}{\partial \phi} \vect{\partial}\phi.
  \end{equation}
  If the normalized basis
  $\left\{ \vect{\partial}\hat{\theta}, \vect{\partial}\hat{\phi} \right\}$ as in Example
  \ref{exam:spherical-coordinate-basis} is adopted, the tangent vector form is
  \begin{equation}
    \nabla f = \frac{1}{r^2} \frac{\partial f}{\partial \theta} r \vect{\partial}\hat{\theta} +
    \frac{1}{r^2\sin^2\theta} \frac{\partial f}{\partial \phi} r\sin\theta
    \vect{\partial}\hat{\phi} = \frac{1}{r} \frac{\partial f}{\partial \theta}
    \vect{\partial}\hat{\theta} + \frac{1}{r\sin\theta} \frac{\partial f}{\partial \phi}
    \vect{\partial}\hat{\phi}.
  \end{equation}
\end{Example}
...
\begin{Example}[Gradient vector on ball manifold]
  ...
  Its tangent vector form is
  \begin{equation}
    \nabla f = \frac{\partial f}{\partial r}\vect{\partial}r + \frac{1}{r^2} \frac{\partial
      f}{\partial \theta} \vect{\partial}\theta + \frac{1}{r^2\sin^2\theta} \frac{\partial
      f}{\partial \phi} \vect{\partial}\phi.
  \end{equation}
  Using the normalized basis
  $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\theta}, \vect{\partial}\hat{\phi}
  \right\}$, we have
  \begin{equation}
    \nabla f = \frac{\partial f}{\partial r}\vect{\partial}\hat{r} + \frac{1}{r} \frac{\partial
      f}{\partial \theta} \vect{\partial}\hat{\theta} + \frac{1}{r\sin\theta} \frac{\partial
      f}{\partial \phi} \vect{\partial}\hat{\phi}.
  \end{equation}
...
\begin{Example}[Gradient vector on cylinder manifold]
  ...
  \begin{equation}
    ...
    \end{pmatrix}.
  \end{equation}
  Using the normalized basis $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\phi},
    \vect{\partial}\hat{z} \right\}$, we have
  \begin{equation}
    \nabla f = \frac{\partial f}{\partial r} \vect{\partial}\hat{r} + \frac{1}{r} \frac{\partial
      f}{\partial \phi} \vect{\partial}\hat{\phi} + \frac{\partial f}{\partial z} \vect{\partial}\hat{z}.
  \end{equation}
\end{Example}
...
\begin{Definition}[Covariant tensor]
  ...
  Cartesian product of $r$ tangent spaces $\otimes^r M_p^n$, i.e.
...
\begin{equation}
  ...
  \begin{aligned}
    Q(v_1,\cdots,v_r) &= Q(\sum_{i_1}v_1^{i_1} \vect{\partial}x^{i_1}, \cdots,
    \sum_{i_r}v_r^{i_r} \vect{\partial}x^{i_r}) \\
    &= \sum_{i_1}v_1^{i_1} Q(\vect{\partial}x^{i_1},\cdots,\sum_{i_r}v_r^{i_r}\vect{\partial}x^{i_r}) \\
    &= \sum_{i_1}\cdots\sum_{i_r}v_1^{i_1}\cdots v_r^{i_r}
    Q(\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_r}),
  \end{aligned}
\end{equation}
where we use $i_k$ as the index to iterative over each component of the $k$-th tangent
vector $v_k$ for $k=1,\cdots,r$. Let
$Q_{i_1\cdots i_r}= Q(\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_r})$, which is the
...
\end{equation}
Apply $\otimes_i\alpha^i$ to $\vect{\partial}x^{k_1},\cdots,\vect{\partial}x^{k_r}$, we have
\begin{equation}
  \left( \otimes_i\alpha^i \right)(\vect{\partial}x^{k_1},\cdots,\vect{\partial}x^{k_r}) =
  \prod_i\alpha^i(\vect{\partial}x^{k_i}) = \prod_i \alpha_{k_i}^{i}.
\end{equation}
Then expand $\prod_i \alpha^i(v_i)$\footnote{Originally, we may want to expand it like this:
  $\prod_i \alpha^i(v_i) = \prod_i \alpha^i\left( \sum_k v_i^k \vect{\partial}x^k \right) =
  \prod_i \sum_k v_i^k \alpha^i(\vect{\partial}x^k)$. However, this hides the fact that for
...
\begin{equation}
  \begin{aligned}
    \prod_i \alpha^i(v_i) &= \prod_i \alpha^i\left( \sum_{k_i}v_i^{k_i}
      \vect{\partial}x^{k_i} \right) = \prod_i \sum_{k_i}v_i^{k_i}
    \alpha^i(\vect{\partial}x^{k_i}) = \prod_i \sum_{k_i} v_i^{k_i} \alpha_{k_i}^i \\
    &= \sum_{k_1}\cdots\sum_{k_r} v_1^{k_1}\cdots v_r^{k_r}
    \alpha_{k_1}^1\cdots\alpha_{k_r}^r = v_1^{k_1}\cdots v_r^{k_r} \left(
      \otimes_i\alpha^i \right)(\vect{\partial}x^{k_1},\cdots,\vect{\partial}x^{k_r}).
  \end{aligned}
\end{equation}
...
Therefore, $\vect{\partial}x^{i_1}\otimes\cdots\otimes \vect{\partial}x^{i_s}$ is a basis
...
\begin{equation}
  ...
  T = T^{i_1\cdots i_s} \vect{\partial}x^{i_1}\otimes\cdots\otimes \vect{\partial}x^{i_s}.
\end{equation}
...
$(dx^{i_1},\cdots,dx^{i_s},\vect{\partial}x^{j_1},\cdots,\vect{\partial}x^{j_r})$, we obtain
\begin{equation}
  W^{i_1\cdots i_s}_{j_1\cdots j_r} = W(dx^{i_1},\cdots,dx^{i_s},\vect{\partial}x^{j_1},\cdots,\vect{\partial}x^{j_r}).
\end{equation}
Then
\begin{equation}
...
\begin{equation}
  ...
  W = W^{i_1\cdots i_s}_{j_1\cdots j_r} \vect{\partial}x^{i_1}\otimes\cdots\otimes
  \vect{\partial}x^{i_s}\otimes dx^{j_1}\otimes\cdots\otimes dx^{j_r}.
\end{equation}
...
\item Coordinate transformation of a covariant tensor
  \begin{equation}
    ...
    Q_{i\cdots j}^V = \left( \frac{\partial x_U^r}{\partial x_V^i} \right) \cdots \left(
      \frac{\partial x_U^s}{\partial x_V^j} \right) Q_{r \cdots s}^U
  \end{equation}
\item Coordinate transformation of a contravariant tensor
  \begin{equation}
    \label{eq:contravariant-tensor-transform}
    T_V^{i \cdots j} = \left( \frac{\partial x_V^i}{\partial x_U^r} \right) \cdots \left(
      \frac{\partial x_V^j}{\partial x_U^s} \right) T_U^{r \cdots s}
  \end{equation}
\item Coordinate transformation of a mixed tensor
  \begin{equation}
    \label{eq:mixed-tensor-transform}
    \left( W^{i \cdots j}_{k \cdots l} \right)_V = \left( \frac{\partial x_V^i}{\partial x_U^c} \right)
    \cdots \left( \frac{\partial x_V^j}{\partial x_U^d} \right) \left( \frac{\partial
        x_U^r}{\partial x_V^k} \right) \cdots \left( \frac{\partial x_U^s}{\partial x_V^l}
...
\begin{Definition}[Contraction of a mixed tensor]
  ...
  Let
  $W_U = \left( W^{i_1 \cdots i_p}_{j_1 \cdots j_q} \right)_U \vect{\partial}x^{i_1}
  \otimes \cdots \otimes \vect{\partial}x^{i_p} \otimes dx^{j_1} \otimes \cdots \otimes
  dx^{j_q}$ be a mixed tensor of contravariant rank $p$ and covariant rank $q$ using the
  coordinate chart $(U,x)$. The contraction of its $k$-th contravariant index and the
  $l$-th covariant index produces a $(p-1)$-th contravariant rank and $(q-1)$-th covariant
  rank tensor, which is defined as
  \begin{equation}
    \label{eq:contraction-of-mixed-tensor}
    \begin{aligned}
      \overset{\circ}{W}_U &= \left( \overset{\circ}{W}^{i_1 \cdots \hat{i}_k \cdots
          i_p}_{j_1 \cdots \hat{j}_l \cdots j_q} \right)_U \vect{\partial}x^{i_1} \otimes
      \cdots \widehat{\vect{\partial}x^{i_k}} \cdots \otimes \vect{\partial}x^{i_p} \otimes
      dx^{j_1}  \otimes \cdots \widehat{dx^{j_l}} \cdots \otimes dx^{j_q} \\
      &= \sum_{m=1}^n \left( W^{i_1 \cdots i_{k-1}, m, i_{k+1} \cdots i_p}_{j_1 \cdots
          j_{l-1}, m, j_{l+1} \cdots j_{q}} \right)_U \vect{\partial}x^{i_1} \otimes \cdots
      \widehat{\vect{\partial}x^{i_k}} \cdots \otimes \vect{\partial}x^{i_p} \otimes dx^{j_1}
...
\begin{Proof}
  Let
  $W_V = \left( W^{r_1 \cdots r_p}_{s_1 \cdots s_q} \right)_V \vect{\partial}y^{r_1}
  \otimes \cdots \otimes \vect{\partial}y^{r_p} \otimes dy^{s_1} \otimes \cdots \otimes
  dy^{s_q}$ be the same mixed tensor as $W_U$ but represented in the coordinate chart
  $(V,y)$. After contracting its $k$-th contravariant index and $l$-th covariant index, it
  becomes
  \begin{align*}
    \overset{\circ}{W}_V
    &= \left( \overset{\circ}{W}^{r_1 \cdots \hat{r}_k \cdots r_p}_{s_1 \cdots \hat{s}_l
      \cdots s_q} \right)_{V} \vect{\partial}y^{r_1}
      \otimes \cdots \widehat{\vect{\partial}y^{r_k}} \cdots \otimes \vect{\partial}y^{r_p}
      \otimes dy^{s_1} \otimes \cdots \widehat{dy^{s_l}} \cdots \otimes dy^{s_q} \\
    &= \sum_{m=1}^n \left( W^{r_1 \cdots r_{k-1}, m, r_{k+1} \cdots
      r_p}_{s_1 \cdots s_{l-1}, m, s_{l+1} \cdots s_q} \right)_V \vect{\partial}y^{r_1}
      \otimes \cdots \widehat{\vect{\partial}y^{r_k}} \cdots \otimes \vect{\partial}y^{r_p}
      \otimes dy^{s_1} \otimes \cdots \widehat{dy^{s_l}} \cdots \otimes dy^{s_q},
  \end{align*}
  ...
  \begin{align*}
    ...
    &= \sum_{m=1}^n \left( \frac{\partial y^{r_1}}{\partial x^{i_1}} \cdots \frac{\partial
      y^{r_{k-1}}}{\partial x^{i_{k-1}}} \frac{\partial y^{m}}{\partial x^{i_k}} \frac{\partial
      y^{r_{k+1}}}{\partial x^{i_{k+1}}} \cdots \frac{\partial y^{r_p}}{\partial x^{i_p}}
      \right) \cdot \\
    & \quad \quad \left( \frac{\partial x^{j_1}}{\partial y^{s_1}} \cdots \frac{\partial
      x^{j_{l-1}}}{\partial y^{s_{l-1}}} \frac{\partial x^{j_l}}{\partial y^{m}} \frac{\partial
      x^{j_{l+1}}}{\partial y^{s_{l+1}}} \cdots \frac{\partial x^{j_q}}{\partial y^{s_q}}
      \right) \left( W^{i_1 \cdots i_{k-1},
      i_k, i_{k+1} \cdots i_p}_{j_1 \cdots j_{l-1}, j_l, j_{l+1} \cdots j_{q}} \right)_U
    \\
    &= \left( \sum_{m=1}^n \frac{\partial y^{m}}{\partial x^{i_k}} \frac{\partial
      x^{j_l}}{\partial y^{m}} \right) \left( \frac{\partial y^{r_1}}{\partial x^{i_1}} \cdots
      \frac{\partial y^{r_{k-1}}}{\partial x^{i_{k-1}}} \frac{\partial y^{r_{k+1}}}{\partial
      x^{i_{k+1}}} \cdots \frac{\partial y^{r_p}}{\partial x^{i_p}} \right) \cdot \\
    & \quad \quad \left( \frac{\partial x^{j_1}}{\partial y^{s_1}} \cdots \frac{\partial
      x^{j_{l-1}}}{\partial y^{s_{l-1}}} \frac{\partial
      x^{j_{l+1}}}{\partial y^{s_{l+1}}} \cdots \frac{\partial x^{j_q}}{\partial y^{s_q}}
      \right) \left( W^{i_1 \cdots i_{k-1},
      i_k, i_{k+1} \cdots i_p}_{j_1 \cdots j_{l-1}, j_l, j_{l+1} \cdots j_{q}} \right)_U,
  \end{align*}
  where
  \begin{equation*}
    \sum_{m=1}^n \frac{\partial y^{m}}{\partial x^{i_k}} \frac{\partial x^{j_l}}{\partial y^{m}} =
    \frac{\partial x^{j_l}}{\partial x^{i_k}} = \delta^{j_l}_{i_k}.
  \end{equation*}
  ...
  \begin{align*}
    \left( \overset{\circ}{W}^{r_1 \cdots \hat{r}_k \cdots r_p}_{s_1 \cdots \hat{s}_l
    \cdots s_q} \right)_{V}
    &= \left( \frac{\partial y^{r_1}}{\partial x^{i_1}} \cdots
      \frac{\partial y^{r_{k-1}}}{\partial x^{i_{k-1}}} \frac{\partial y^{r_{k+1}}}{\partial
      x^{i_{k+1}}} \cdots \frac{\partial y^{r_p}}{\partial x^{i_p}} \right) \cdot \\
    & \quad \quad \left( \frac{\partial x^{j_1}}{\partial y^{s_1}} \cdots \frac{\partial
      x^{j_{l-1}}}{\partial y^{s_{l-1}}} \frac{\partial
  ...
  mixed tensor. \emph{This can be considered as an internal interaction in a mixed tensor,
    which consumes or eliminates a pair of indices.}
\end{Proof}
...
\begin{equation}
  ...
  \alpha(v_{1},\cdots,v_{k}) = \alpha \left(
    \sum_{i_1}v_1^{i_1}\vect{\partial}x^{i_1},\cdots,\sum_{i_k}v_k^{i_k}\vect{\partial}x^{i_k}
  \right) = \sum_{i_1}\cdots\sum_{i_k}v_1^{i_1}\cdots v_k^{i_k}
  \alpha(\vect{\partial}x^{i_1},\cdots, \vect{\partial}x^{i_k}).
\end{equation}
Moreover, due to the antisymmetry of $\alpha$, some terms in the above multi-summation can
be combined. For example, because
\begin{equation}
  \alpha(\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_r},\cdots,\vect{\partial}x^{i_s},\cdots,\vect{\partial}x^{i_k})
  =
  -\alpha(\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_s},\cdots,\vect{\partial}x^{i_r},\cdots,\vect{\partial}x^{i_k}),
\end{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    = & \left( v_1^{i_1} \cdots v_r^{i_r} \cdots v_s^{i_s} \cdots v_k^{i_k} - v_1^{i_1}
      \cdots v_r^{i_s} \cdots v_s^{i_r} \cdots v_k^{i_k} \right) \alpha(\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_r},\cdots,\vect{\partial}x^{i_s},\cdots,\vect{\partial}x^{i_k}).
  \end{aligned}
\end{equation}
For simplicity, we can write the tuple of basis vectors
$\left(
  \vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_r},\cdots,\vect{\partial}x^{i_s},\cdots,\vect{\partial}x^{i_k}
\right)$ as $\vect{\partial}x^{I}$, where $I$ is a tuple of indices $I = (i_1,\cdots,i_k)$
...
$\alpha(\vect{\partial}x^I) = \alpha_I$ and $\alpha(\vect{\partial}x^J) = \alpha_J$, if the
...
(\ref{eq:k-form-applied-to-k-vector-expanded}) is $C_n^k$, which is the number of ways
...
applying it to all such tuples of basis vectors $\vect{\partial}x^I$ of the tangent space
...
\item When $\alpha$ is a $n$-form, $C_n^n = 1$ and there is only term in the summation,
  which contains $\alpha(\vect{\partial}x^1,\cdots,\vect{\partial}x^n)$.
...
evaluation changes sign, which satisfies anti-symmetry. Similar result holds for the case
when $i,j$ are in the range $k+1,\cdots,k+l$. However, if $i$ is selected from
$1,\cdots,k$ and $j$ is selected from $k+1,\cdots,k+l$, swapping $v_i$ and $v_j$ will make
$\alpha\otimes\beta$ not antisymmetric. Therefore, a standard tensor product of two
exterior forms is not a closed operation and we need to define a new type of product which
ensures the antisymmetric property.
...
\begin{Definition}[Exterior (or wedge or Grassmann) product]
  ...
  $\left( \alpha\wedge\beta \right)(v_I)$ is zero\footnote{Here we do not have requirement
    ...
    $\vect{\partial}x^I = \left( \vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_{l+m}}
...
\begin{Remark}
  ...
  \item $J$ and $K$ comprise a partition of the index set $I$, each of which is an
    increasing set of indices. Hence, the total number of terms in the summation
  ...
  \item If the tuple of $l+m$ tangent vectors are simply basis vectors of the tangent
    space, i.e. $\vect{\partial}x^I =
    (\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_{l+m}})$, Equation
...
\begin{Proof}
  ...
  $(\vect{\partial}x^{n_1},\cdots,\vect{\partial}x^{l+m+r})$ be a tuple of basis vectors in
...
\begin{Example}[Apply exterior product of cotangent space basis vectors to tangent
  ...
  $\vect{\partial}x^{i_1},\cdots,\vect{\partial}x^{i_k}$ of the tangent space. Then the
...
\begin{Example}[Apply exterior product of cotangent space basis vectors to tangent space
  ...
  tangent space $M_p^n$:
  $\vect{\partial}x^J \coloneqq (\vect{\partial}x^{j_1},\cdots,\vect{\partial}x^{j_k})$. Then we
  apply $dx^I$ to $\vect{\partial}x^J$ and use Equation
  (\ref{eq:1-form-exterior-product-applied-to-k-vector}) and
  (\ref{eq:projection-operator}):
  \begin{equation}
    dx^I(\vect{\partial}x^J) = \sum_{L} \delta_J^L dx^{i_1}(\vect{\partial}x^{l_1}) \cdots
    dx^{i_k}(\vect{\partial}x^{l_k}) = \sum_L \delta_J^L \delta_{l_1}^{i_1} \cdots
    \delta_{l_k}^{i_k}.
  \end{equation}
  ...
  \begin{equation}
    ...
    dx^I(\vect{\partial}x^J) = \delta_J^I.
  \end{equation}
\end{Example}
...
\begin{Proposition}
  ...
  \begin{equation}
    \alpha^k = \sum_{\underaccent{\rightharpoondown}{I}} \alpha^k(\vect{\partial}x^I) dx^I.
  \end{equation}
\end{Proposition}
...
\begin{Proof}
  ...
  tangent space: $\vect{\partial}x^J$. If the equality holds for any $J$, the proposition is
  proved.

  The left hand side is simply $\alpha^k(\vect{\partial}x^J)$. The right hand side is
  \begin{equation}
    \sum_{\underaccent{\rightharpoondown}{I}}\alpha^k(\vect{\partial}x^I)dx^I(\vect{\partial}x^J)
    = \sum_{\underaccent{\rightharpoondown}{I}} \alpha^k(\vect{\partial}x^I)\delta_J^I.
  \end{equation}
  ...
  $J$, $\delta_J^{I_0}=1$ and
  $\alpha^k(\vect{\partial}x^{I_0}) = \alpha^k(\vect{\partial}x^J)$. Hence, the right hand
  side is equal to the left hand side. When $I_0$ is an odd permutation of $J$,
  $\delta_J^{I_0}=-1$ and $\alpha^k(\vect{\partial}x^{I_0}) = -\alpha^k(\vect{\partial}x^J)$.
...
$\alpha^k$ to the set of basis vectors $\vect{\partial}x^I$ in the tangent space $M_p^n$
...
\begin{Definition}[Exterior differentiation]
  ...
  \begin{equation}
    d\alpha^{0} = \frac{\partial a(x)}{\partial x^j} dx^j.
  \end{equation}
  ...
  \begin{Proof}
    ...
    \begin{align*}
      ...
      \intertext{In a coordinate chart $(U,x)$:}
               &= d \left( \sum_{\underaccent{\rightharpoondown}{K}} \frac{\partial
                 \alpha_{K}(x)}{\partial x^i} dx^i \wedge dx^K \right) \\
      \intertext{N.B. Einstein summation notation is adopted above and the summation with respect to $i$ is
      omitted.}
               &= \sum_{\underaccent{\rightharpoondown}{K}} d \left( \frac{\partial
                 \alpha_K(x)}{\partial x^i} \right) dx^i \wedge dx^K =
                 \sum_{\underaccent{\rightharpoondown}{K}} \frac{\partial^2 \alpha_K(x)}{\partial x^i
                 \partial x^j} dx^i \wedge dx^j \wedge dx^K. \\
    \end{align*}
    For any pair of different $i$ and $j$,
    $\frac{\partial^2 \alpha_K(x)}{\partial x^i \partial x^j}$ is equal to
    $\frac{\partial^2 \alpha_K(x)}{\partial x^j \partial x^i}$ and according to Equation
...
\sum_{\underaccent{\rightharpoondown}{J}} d\alpha^k(\vect{\partial}x^J) dx^J$, where $J$ is
...
\begin{align*}
  ...
  $d\alpha_I$ to $\vect{\partial}x^j$ and $dx^I$ apply to
  $\vect{\partial}x^L$, where $L$ is an increasing index set $L =
  \underaccent{\rightharpoondown}{J\backslash\{j\}}$. Hence,}
                &= \sum_{\underaccent{\rightharpoondown}{I}} \sum_j \sum_{\underaccent{\rightharpoondown}{L}} \delta_J^{jL}
                  d\alpha_I(\vect{\partial}x^j) dx^I(\vect{\partial}x^L) \\
  \intertext{According to Equation (\ref{eq:cotangent-basis-appled-to-tangent-basis}):}
                &= \sum_{\underaccent{\rightharpoondown}{I}} \sum_j
                  \sum_{\underaccent{\rightharpoondown}{L}} \delta_J^{jL} \frac{\partial
                  \alpha_I(x)}{\partial x^j} \delta_L^I
\end{align*}
...
\begin{equation}
  ...
  \frac{\partial \alpha_L(x)}{\partial x^j}.
\end{equation}
...
\begin{equation}
  ...
  (d\alpha^k)_J = \sum_j \delta_J^{jL} \frac{\partial \alpha_L(x)}{\partial x^j}.
\end{equation}
...
\begin{equation}
  ...
  (d\alpha^k)_J = \frac{\partial
    \alpha_{\underaccent{\rightharpoondown}{J\backslash\{j_1\}}}}{\partial x^{j_1}} -
  \frac{\partial \alpha_{\underaccent{\rightharpoondown}{J\backslash\{j_2\}}}}{\partial
    x^{j_2}} + \frac{\partial
    \alpha_{\underaccent{\rightharpoondown}{J\backslash\{j_3\}}}}{\partial x^{j_3}} - \cdots
  = \sum_{r=1}^{k+1} (-1)^{r-1} \frac{\partial
    \alpha_{\underaccent{\rightharpoondown}{J\backslash\{j_r\}}}}{\partial x^{j_r}}.
\end{equation}
...
\begin{Example}[Exterior differential of a 1-form]
  ...
  \begin{equation}
    ...
    (d\alpha^1)_{J} = \delta_{J}^{ij} \frac{\partial \alpha_j(x)}{\partial x^i} +
    \delta_{J}^{ji} \frac{\partial \alpha_i(x)}{\partial x^j} = \frac{\partial
      \alpha_j(x)}{\partial x^i} - \frac{\partial \alpha_i(x)}{\partial x^j}.
  \end{equation}
\end{Example}
...
\begin{Example}[Exterior differential of a 1-form when the manifold dimension is 3]
  ...
  \begin{equation}
    ...
    \begin{aligned}
      (d\alpha^1) &= \left( \frac{\partial \alpha_2}{\partial x^1} - \frac{\partial
          \alpha_1}{\partial x^2} \right) dx^1 \wedge dx^2 + \left( \frac{\partial
          \alpha_3}{\partial x^2} - \frac{\partial \alpha_2}{\partial x^3} \right) dx^2 \wedge
      dx^3 + \left( \frac{\partial \alpha_3}{\partial x^1} - \frac{\partial \alpha_1}{\partial x^3}
      \right) dx^1 \wedge dx^3 \\
      &= \left( \frac{\partial \alpha_2}{\partial x^1} - \frac{\partial
          \alpha_1}{\partial x^2} \right) dx^1 \wedge dx^2 + \left( \frac{\partial
          \alpha_3}{\partial x^2} - \frac{\partial \alpha_2}{\partial x^3} \right) dx^2 \wedge
      dx^3 + \left( \frac{\partial \alpha_1}{\partial x^3} - \frac{\partial \alpha_3}{\partial x^1}
...
\begin{Example}[Exterior differential of a 2-form]
  ...
  \begin{equation}
    ...
    \begin{aligned}
      (d\beta^2)_{J} &= \delta_J^{ijk} \frac{\partial \beta_{jk}(x)}{\partial x^i} +
      \delta_J^{jik} \frac{\partial \beta_{ik}(x)}{\partial x^j} + \delta_J^{kij} \frac{\partial
        \beta_{ij}(x)}{\partial x^k} \\
      &= \frac{\partial \beta_{jk}(x)}{\partial x^i} - \frac{\partial \beta_{ik}(x)}{\partial x^j}
      + \frac{\partial \beta_{ij}(x)}{\partial x^k} = \frac{\partial \beta_{jk}(x)}{\partial x^i}
      + \frac{\partial \beta_{ki}(x)}{\partial x^j} + \frac{\partial \beta_{ij}(x)}{\partial x^k}.
    \end{aligned}
  \end{equation}
\end{Example}
...
\begin{Example}[Exterior differential of a 2-form when the manifold dimension is 3]
  ...
  term:
  \begin{equation}
    (d\beta^2)_{123} = \delta_{123}^{123} \frac{\partial \beta_{23}}{\partial x^1} +
    \delta_{123}^{213} \frac{\partial \beta_{13}}{\partial x^2} + \delta_{123}^{312}
    \frac{\partial \beta_{12}}{\partial x^3} = \frac{\partial \beta_{23}}{\partial x^1} -
    \frac{\partial \beta_{13}}{\partial x^2} + \frac{\partial \beta_{12}}{\partial x^3}.
  \end{equation}
  ...
  \begin{equation}
    d\beta^2 = \left( \frac{\partial \beta_{23}}{\partial x^1} + \frac{\partial \beta_{31}}{\partial
        x^2} + \frac{\partial \beta_{12}}{\partial x^3} \right) dx^1 \wedge dx^2 \wedge dx^3.
  \end{equation}
  ...
  \begin{equation}
    ...
    d\beta^2 = \left( \frac{\partial b_1}{\partial x^{1}} + \frac{\partial b_2}{\partial x^2} +
      \frac{\partial b_3}{\partial x^3} \right) dx^1 \wedge dx^2 \wedge dx^3.
  \end{equation}
...
\begin{Example}[Exterior differential of a 3-form when the manifold dimension is 4]
  ...
  differential is
  \begin{equation}
    d\beta^3 = \left( \frac{\partial b_1}{\partial x^1} - \frac{\partial b_2}{\partial x^2} +
      \frac{\partial b_3}{\partial x^3} - \frac{\partial b_4}{\partial x^4} \right) dx^1 \wedge dx^2
...
\begin{Example}[Exterior differential of a $(n-1)$-form when the manifold dimension is $n$]
  ...
  differential is
  \begin{equation}
    d\beta^{n-1} = \left( \sum_{r=1}^n (-1)^{r-1} \frac{\partial
        \beta_{(1,\cdots,\hat{r},\cdots,n)}}{\partial x^r}
    \right) dx^{(1,\cdots,n)}.
  \end{equation}
  ...
  \begin{equation}
    d\beta^{n-1} = \left( \sum_{r=1}^n (-1)^{(r-1)(n-1)} \frac{\partial
        \beta_{(r+1,\cdots,n,1,\cdots,r-1)}}{\partial x^r}
    \right) dx^{(1,\cdots,n)}.
  \end{equation}
  When the manifold dimension $n$ (assume it is larger than 2) is odd, the sign of the
  term $\partial \beta_{(r+1,\cdots,n,1,\cdots,r-1)}/\partial x^r$ in the above sum, i.e.
...
\begin{Example}[Exterior differential of $d\alpha^1$ when the manifold dimension is 3]
  ...
  \begin{equation}
    \begin{aligned}
      dd\alpha^1 &= \left( \frac{\partial (d\alpha^1)_{23}}{\partial x^1} + \frac{\partial
          (d\alpha^1)_{31}}{\partial x^2} + \frac{\partial (d\alpha^1)_{12}}{\partial x^3}
      \right) dx^1 \wedge dx^2 \wedge
      dx^3 \\
      &= \left( \frac{\partial^2 \alpha_3}{\partial x^2 \partial x^1} - \frac{\partial^2
          \alpha_2}{\partial x^3 \partial x^1} + \frac{\partial^2 \alpha_1}{\partial x^3 \partial
          x^2} - \frac{\partial^2 \alpha_3}{\partial x^1 \partial x^2} + \frac{\partial^2
          \alpha_2}{\partial x^1 \partial x^3} - \frac{\partial^2 \alpha_1}{\partial x^2 \partial
...
\item Pull-back operator $F^{*}$ and exterior derivative $d$ are commutative.
  ...
  \begin{Proof}
    ...
    \item When $\alpha$ is a 0-form, $F^{*}\alpha$ is also a 0-form with respect to the
      ...
      \begin{equation*}
        d(F^{*}\alpha)(v_x) = \frac{\partial (F^{*}\alpha)(x)}{\partial x^i} v_x^i.
      \end{equation*}
      ...
      we have
      \begin{equation*}
        d(F^{*}\alpha)(v_x) = \frac{\partial \alpha(y(x))}{\partial x^i} v_x^i = \frac{\partial
          \alpha(y)}{\partial y^j} \frac{\partial y^j}{\partial x^i} v_x^i.
      \end{equation*}
      Refer to the expression of contravariant transformation in Equation
      (\ref{eq:contravariant-transformation}), $\frac{\partial y^j}{\partial x^i}v_x^i$ is the
      $j$-th component of the tangent vector $v_y$ in $M_{p(y)}^n$, which is $dy^j(v_y)$.
      Therefore,
      \begin{equation*}
        d(F^{*}\alpha)(v_x) = \frac{\partial \alpha(y)}{\partial y^j} dy^j(v_y) = \left(
          \frac{\partial \alpha(y)}{\partial y^j} dy^j \right)(v_y).
      \end{equation*}
...
\begin{Proposition}[Expression of the pull-back of a basis vector in $\bigwedge^k M_p^{n*}$]
  ...
  \begin{equation}
    ...
    F^{*}dy^J = \sum_{\underaccent{\rightharpoondown}{L}} \det \left\{
      \frac{\partial y^J}{\partial x^L} \right\} dx^L.
  \end{equation}
\end{Proposition}
...
\begin{Proof}
  ...
  \begin{equation*}
    \left( F^{*} dy^{j_l} \right)(v_x) = dy^{j_l}(F_{*} v_x) = \frac{\partial y^{j_l}}{\partial
      x^i} v_x^i = \frac{\partial y^{j_l}}{\partial x^i} dx^i(v_x) = \left( \frac{\partial
        y^{j_l}}{\partial x^i} dx^i \right)(v_x).
  \end{equation*}
  ...
  \begin{equation}
    ...
    F^{*}dy^{j_l} = \frac{\partial y^{j_l}}{\partial x^{i_l}} dx^{i_l}.
  \end{equation}
  ...
  \begin{align*}
    ...
    &= \left(
      \frac{\partial y^{j_1}}{\partial x^{i_1}} dx^{i_1} \right) \wedge \cdots \wedge \left(
      \frac{\partial y^{j_k}}{\partial x^{i_k}} dx^{i_k} \right) \\
    &= \sum_I \left( \frac{\partial y^{j_1}}{\partial x^{i_1}} \cdots \frac{\partial
      y^{j_k}}{\partial x^{i_k}} \right) dx^I.
  \end{align*}
  ...
  the basis $\vect{\partial}x^L$ of the product tangent space, where $L=(l_1,\cdots,l_k)$ is
  an increasing index set.
  \begin{align*}
    \left( F^{*}dy^J \right)(\vect{\partial}x^L)
    &= \left[ \sum_I \left( \frac{\partial
      y^{j_1}}{\partial x^{i_1}} \cdots \frac{\partial
      y^{j_k}}{\partial x^{i_k}} \right) dx^I \right](\vect{\partial}x^L) \\
    &= \sum_I \left( \frac{\partial y^{j_1}}{\partial x^{i_1}} \cdots \frac{\partial
      y^{j_k}}{\partial x^{i_k}} \right) dx^I(\vect{\partial}x^L) \\
    \intertext{Apply Equation (\ref{eq:cotangent-basis-appled-to-tangent-basis})}
    &= \sum_I \left( \frac{\partial y^{j_1}}{\partial x^{i_1}} \cdots \frac{\partial
      y^{j_k}}{\partial x^{i_k}} \right) \delta_L^I.
  \end{align*}
  ...
  determinant
  \begin{equation*}
    \det \left( \frac{\partial y^{j_r}}{\partial x^{l_s}} \right) = \det \left\{
      \frac{\partial y^J}{\partial x^L} \right\}.
  \end{equation*}
...
\begin{Proof}
  \begin{align*}
    ...
    &= \sum_{\underaccent{\rightharpoondown}{J}} \alpha_J(y(x))
      \sum_{\underaccent{\rightharpoondown}{L}} \det \left\{ \frac{\partial y^J}{\partial x^L}
      \right\} dx^L \\
    &= \sum_{\underaccent{\rightharpoondown}{L}} \left(
      \sum_{\underaccent{\rightharpoondown}{J}} \alpha_J(y(x)) \det \left\{ \frac{\partial
      y^J}{\partial x^L} \right\} \right) dx^L.
  \end{align*}
  Therefore, the expansion coefficient $\alpha_L^{*} = (F^{*}\alpha^k)(\vect{\partial}x^L)$
  is
  \begin{equation*}
    \alpha_L^{*} = \sum_{\underaccent{\rightharpoondown}{J}} \alpha_J(y(x)) \det \left\{
      \frac{\partial y^{J}}{\partial x^L} \right\}.
...
\begin{Example}[Pull-back of the inclusion map]
  ...
  \begin{align*}
    \vect{i}^{*}\beta^2
    &= \left( b_1(x(u)) \det \left\{ \frac{\partial (x^2, x^3)}{\partial (u^1,u^2)} \right\} -
      b_2(x(u)) \det \left\{ \frac{\partial (x^1,x^3)}{\partial (u^1,u^2)} \right\} +
      b_3(x(u)) \det \left\{ \frac{\partial (x^1,x^2)}{\partial (u^1,u^2)} \right\} \right)
      du^1 \wedge du^2 \\
    &= \left( b_1(x(u)) \det \left\{ \frac{\partial (x^2, x^3)}{\partial (u^1,u^2)} \right\} +
      b_2(x(u)) \det \left\{ \frac{\partial (x^3,x^1)}{\partial (u^1,u^2)} \right\} +
      b_3(x(u)) \det \left\{ \frac{\partial (x^1,x^2)}{\partial (u^1,u^2)} \right\} \right)
      du^1 \wedge du^2.
  \end{align*}
  ...
  $\left\{ \vect{\partial}u^1, \vect{\partial}u^2 \right\}$ of the tangent space $M_{p(u)}^2$
  define a normal vector $\vect{n}$ via the cross product of their representations using
  the basis $\left\{ \vect{\partial}x^1, \vect{\partial}x^2, \vect{\partial}x^3 \right\}$ of the
  ...
  $\left\{ \vect{\partial}u^1,\vect{\partial}u^2 \right\}$ may not be an orthonormal basis,
  ...
  \begin{equation*}
    \vect{n} = \begin{pmatrix}
      \frac{\partial x^1}{\partial u^1} \\ \frac{\partial x^2}{\partial u^1} \\ \frac{\partial
        x^3}{\partial u^1}
    \end{pmatrix}
    \times \begin{pmatrix}
      \frac{\partial x^1}{\partial u^2} \\ \frac{\partial x^2}{\partial u^2} \\ \frac{\partial
        x^3}{\partial u^2}
    \end{pmatrix}.
  \end{equation*}
...
\begin{Example}[Pull-back of the identity map in $M^3$]
  ...
  \begin{equation*}
    \vect{I}^{*}\alpha^3 = a(x(u)) \det \left\{ \frac{\partial x}{\partial u} \right\} du^1
...
\begin{Definition}[Orientation of a vector space]
  ...
  \end{equation}
  Therefore, for any $i$ in the range $1,\cdots,n$, $\vect{f}_i = \vect{e}_j P^j_i$. If
...
$(\vect{\partial}x^1,\cdots,\vect{\partial}x^n)$. Even though all coordinate charts in the
...
\begin{Definition}[Orientation of a manifold]
  ...
  $(\vect{\partial}x^1,\cdots,\vect{\partial}x^n)$. For any point $p$ in the overlap of two
  charts $(U,x)$ and $(V,y)$, the orientation of the tangent space $M_p^n$ can also be
  determined from the basis $(\vect{\partial}y^1,\cdots,\vect{\partial}y^n)$. According to
  Equation (\ref{eq:tangent-space-bases-relation}), the relationship between
  $\vect{\partial}x$ and $\vect{\partial}y$ is
  \begin{equation}
    \left( \partial y^1,\cdots,\partial y^n \right) = \left( \partial
      x^1,\cdots,\partial x^n  \right) \left[ \frac{\partial x}{\partial y} \right].
  \end{equation}
  If the Jacobian determinant $\abs{\frac{\partial x}{\partial y}}$ is positive, according
  to Definition \ref{def:vector-space-orientation}, the two orientations are equivalent.
  If all pairs of overlapping coordinate charts have equivalent orientations, the manifold
  $M^n$ is orientable and has a definite orientation.
\end{Definition}

\begin{Definition}[Orientation function]
  For a coordinate chart $(U,x)$ of the manifold $M^n$, an orientation function
  $o(\vect{\partial}x^1,\cdots,\vect{\partial}x^n)$ is defined to operate on the basis of the
...
\begin{Definition}[Pseudo $k$-form]
  ...
  $(\vect{\partial}x^1,\cdots,\vect{\partial}x^n)$ has the positive orientation, i.e.
...
$\vect{\partial}y = (\vect{\partial}y^1,\cdots,\vect{\partial}y^n)$, where
\begin{equation*}
  \begin{cases}
    \vect{\partial}y^{i_0} = \vect{\partial}x^{j_0}, \vect{\partial}y^{j_0} =
    \vect{\partial}x^{i_0} \\
    \vect{\partial}y^i = \vect{\partial}x^i & (i \neq i_0, i\neq j_0)
  \end{cases}.
\end{equation*}
Hence, the new basis $\vect{\partial}y$ is obtained by swapping the $i_0$-th and $j_0$-th
components in the original basis $\vect{\partial}x$. Since the tuple of vectors
$(v_1,\cdots,v_n)$ are physical entities which maintain their lengths and directions in
the tangent space $M_p^n$, when the new basis $\vect{\partial}y$ is adopted, their numerical
...
orientation\footnote{Once the coordinate chart of a neighborhood of the point $p$ in the
  ...
  space $M_p^n$ rooted at $p$ is settled, which is $(\partial x^1, \cdots, \partial x^n)$, and
...
\begin{Definition}[Volume $n$-form with respect to $n$-dimensional manifold $M^n$]
  ...
  $\vect{\partial}x = (\vect{\partial}x^1,\cdots,\vect{\partial}x^n)$ of the tangent space
  ...
  \end{equation}
\end{Definition} 
Apply this volume $n$-form to the basis $\vect{\partial}x$ and use Equation
(\ref{eq:cotangent-basis-appled-to-tangent-basis}), we know that the volume spanned by the
basis $\vect{\partial}x$ is $o(x)$, i.e. \emph{the volume spanned by a set of positively
  oriented orthonormal basis vectors is 1.}

Let $(y)$ be an arbitrary coordinate chart containing $p$. The basis $\vect{\partial}y$ for
...
\begin{equation*}
  \mathrm{vol}^n = o(x) dx^1 \wedge \cdots \wedge dx^n = o(x) \det \left\{  \frac{\partial x}{\partial
      y} \right\} dy^1 \wedge \cdots \wedge dy^n.
\end{equation*}
When $M_{p(y)}^n$ has the positive orientation, $\mathrm{vol}_{(y)}^n(\vect{\partial}y)$
...
\begin{equation}
  ...
  \mathrm{vol}^n = o(y) \Bigg\vert\det \left\{ \frac{\partial x}{\partial y}
...
\begin{equation*}
  g_{rs}^{(y)} = \frac{\partial x^i}{\partial y^r} \frac{\partial x^j}{\partial y^s} g_{ij}^{(x)}
  = \frac{\partial x^i}{\partial y^r} \frac{\partial x^j}{\partial y^s} \delta_{ij} = \sum_i
  \frac{\partial x^i}{\partial y^r} \frac{\partial x^i}{\partial y^s}.
\end{equation*}
Define $g(y) \coloneqq \det(g_{rs}^{(y)})$, we have
\begin{equation*}
  g(y) = \det \left\{ \sum_i
    \frac{\partial x^i}{\partial y^r} \frac{\partial x^i}{\partial y^s} \right\} = \det \left\{
    \left[ \frac{\partial x}{\partial y}^{\mathrm{T}} \right] \left[ \frac{\partial x}{\partial y}
    \right] \right\} = \det \left\{ \frac{\partial x}{\partial y} \right\}^2.
\end{equation*}
...
\begin{Remark}
  ...
  orthonormal basis $\vect{\partial}x$. However, this is not mandatory. Generally, a volume
  $n$-form is
  \begin{equation}
...
$\vect{\partial}y^i = -\vect{\partial}x^i$, the numerical representation of $u$ in this new
...
\begin{Proof}
  Assume the coordinate chart is $(x)$, let $v=v^i \vect{\partial}x^i$. Then
  \begin{equation}
    (i_v\alpha^k)_J = \alpha^k(v, \vect{\partial}x^{j_2},\cdots,\vect{\partial}x^{j_k}) =
    \alpha^k(v^i \vect{\partial}x^i, \vect{\partial}x^{j_2},\cdots,\vect{\partial}x^{j_k}) = v^i
    \alpha^k(\vect{\partial}x^i, \vect{\partial}x^{j_2},\cdots,\vect{\partial}x^{j_k}) = v^i\alpha^k_{i,J}.
  \end{equation}
...
\item The exterior differential of a 0-form $f$, i.e.
  $df = \frac{\partial f}{\partial x^j} dx^j$, is equivalent to the gradient of the scalar
  ...
  \begin{equation}
    df = \flat(\nabla f), (df)_j = \frac{\partial f}{\partial x^j} = g_{ji} \left( \nabla f
    \right)^i, df^{\mathrm{T}} = G\nabla f.
  \end{equation}

  \begin{equation}
    \label{eq:1-form-to-gradient}
    \nabla f = \sharp(df), (\nabla f)^i = g^{ij} (df)_j = g^{ij} \frac{\partial f}{\partial
      x^{i}}, \nabla f = G^{-1} df^{\mathrm{T}}.
  \end{equation}
...
\item A tangent vector $v$ can be identified with a pseudo $(n-1)$-form, which is obtained
  ...
  chart $(x)$. Let $v$ be a tangent vector $v = v^j \vect{\partial}u^j$. The interior
  product of $\mathrm{vol}^n$ with respect to $v$ is
  \begin{align*}
    i_v \mathrm{vol}^n
    &= i_v \left( o(u) \rho(u) du^1 \wedge \cdots \wedge du^n \right) \\
    &= i_{v^j \vect{\partial}u^j} \left( o(u) \rho(u) du^1 \wedge \cdots \wedge du^n \right)
    \\
    ...
    &= o(u) \rho(u) v^j i_{\vect{\partial}u^j} \left( du^1 \wedge \cdots \wedge du^n \right)
    \\
    ...
    &= o(u) \rho(u) v^j \left( i_{\vect{\partial}u^j}(du^1) \wedge \cdots \wedge du^n - du^1
      \wedge i_{\vect{\partial}u^j}\left( du^2 \wedge \cdots \wedge du^n \right) \right) \\
    &= o(u) \rho(u) v^j \left( i_{\vect{\partial}u^j}(du^1) \wedge \cdots \wedge du^n -
      du^1 \wedge i_{\vect{\partial}u^j}(du^2) \wedge \cdots \wedge du^n + du^1 \wedge du^2
      \wedge i_{\vect{\partial}u^j}\left( du^3 \wedge \cdots \wedge du^n \right) \right) \\
    &= o(u) \rho(u) v^j \left( \sum_{i=1}^n (-1)^{i-1} du^1 \wedge \cdots
      \wedge i_{\vect{\partial}u^j}(du^i) \wedge \cdots \wedge du^n \right) \\
  ...
  orthonormal basis $\vect{\partial}x$. In this space, the pseudo volume 3-form using the
...
\item The cross product of two tangent vectors $u$ and $v$ in $M_p^3$
  ...
  \item It is defined as a pseudo vector (see Section \ref{sec:pseudo-vector}) which
    ...
    $\left[ \frac{\partial x}{\partial y} \right]^{\mathrm{T}} \left[ \frac{\partial x}{\partial y}
    ...
    \begin{Proof}
      ...
      \begin{equation*}
        i_{u\times v} \mathrm{vol}^3 = \sqrt{g(y)} \sqrt{g(y)} G^{-1} \begin{pmatrix}
          ...
          u^3v^1-u^1v^3 \\
          u^1v^2-u^2v^1
        \end{pmatrix} = \det\left\{\left[ \frac{\partial x}{\partial y} \right]^{\mathrm{T}} \left[
            \frac{\partial x}{\partial y} \right]\right\} G^{-1} \begin{pmatrix}
          u^2v^3-u^3v^2 \\
          u^3v^1-u^1v^3 \\
          u^1v^2-u^2v^1
        \end{pmatrix}.
      ...
      \begin{equation*}
        G^{-1} = \frac{\adj(G)}{\det(G)} = \frac{\begin{pmatrix}
          ...
          g_{21}g_{32} - g_{31}g_{22} & g_{31}g_{12} - g_{11}g_{32} & g_{11}g_{22} -
          g_{21}g_{12}
        \end{pmatrix}}{\det\left\{\left[ \frac{\partial x}{\partial y} \right]^{\mathrm{T}}
          \left[ \frac{\partial x}{\partial y} \right]\right\}}.
    \end{equation*}
    Then
    \begin{align*}
...
\item Curl of a vector $a$: like the definition of cross product which depends on the
  ...
  \begin{equation}
    ...
    \curl a = \frac{1}{\sqrt{g(y)}} \begin{pmatrix}
      \frac{\partial \alpha_3}{\partial y^2} - \frac{\partial \alpha_2}{\partial y^3} \\
      \frac{\partial \alpha_1}{\partial y^3} - \frac{\partial \alpha_3}{\partial y^1} \\
      \frac{\partial \alpha_2}{\partial y^1} - \frac{\partial \alpha_1}{\partial y^2}
    \end{pmatrix},
  \end{equation}
  ...
  \begin{equation}
    ...
    \curl a = \begin{pmatrix}
      \frac{\partial a^3}{\partial x^2} - \frac{\partial a^2}{\partial x^3} \\
      \frac{\partial a^1}{\partial x^3} - \frac{\partial a^3}{\partial x^1} \\
      \frac{\partial a^2}{\partial x^1} - \frac{\partial a^1}{\partial x^2}
    \end{pmatrix}.
  \end{equation}
  
  In the ball coordinate chart $(r,\theta,\phi)$ with the non-normalized basis
  $\left\{ \vect{\partial}r, \vect{\partial}\theta, \vect{\partial}\phi \right\}$,
  \begin{equation}
    \label{eq:curl-expr-ball-coordinate-nonnormalized-basis}
    \curl a = \begin{pmatrix}
      \sin\theta \frac{\partial a^{3}}{\partial \theta} - \frac{1}{\sin\theta} \frac{\partial
        a^{2}}{\partial \phi} + 2a^3\cos\theta \\
      -\frac{2\sin\theta}{r}a^3 - \sin\theta \frac{\partial a^3}{\partial r} +
      \frac{1}{r^2\sin\theta}\frac{\partial a^1}{\partial \phi}\\
      \frac{2}{r\sin\theta}a^2 - \frac{1}{r^2\sin\theta}\frac{\partial a^1}{\partial \theta} +
      \frac{1}{\sin\theta}\frac{\partial a^{2}}{\partial r}
    \end{pmatrix}.
  \end{equation}
  If the normalized basis
  $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\theta}, \vect{\partial}\hat{\phi}
  ...
  \begin{equation}
    ...
    \curl a = \begin{pmatrix}
      \frac{\cos\theta}{r\sin\theta} a^3 - \frac{1}{r\sin\theta} \frac{\partial
        a^{2}}{\partial \phi} + \frac{1}{r} \frac{\partial a^{3}}{\partial \theta} \\
      \frac{1}{r\sin\theta} \frac{\partial a^{1}}{\partial \phi} - \frac{1}{r}a^3 -
      \frac{\partial a^{3}}{\partial r} \\
      \frac{1}{r}a^2 - \frac{1}{r} \frac{\partial a^{1}}{\partial \theta} + \frac{\partial
        a^{2}}{\partial r}
    \end{pmatrix}.
  \end{equation}
  
  In the cylindrical coordinate chart $(r,\phi,z)$ with the non-normalized basis $\left\{
    \vect{\partial}r, \vect{\partial}\phi, \vect{\partial}z \right\}$,
  \begin{equation}
    \label{eq:curl-expr-cylindrical-coordinate-nonnormalized-basis}
    \curl a = \begin{pmatrix}
      \frac{1}{r}\frac{\partial a^{3}}{\partial \phi} - r \frac{\partial a^{2}}{\partial z} \\
      \frac{1}{r}\frac{\partial a^{1}}{\partial z} - \frac{1}{r}\frac{\partial a^{3}}{\partial r}
      \\
      r \frac{\partial a^{2}}{\partial r} - \frac{1}{r} \frac{\partial a^{1}}{\partial \phi} +
      2a^2
    \end{pmatrix}.
  \end{equation}
  If the normalized basis $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\phi},
    \vect{\partial}\hat{z} \right\}$ is adopted,
  \begin{equation}
    \label{eq:curl-expr-cylindrical-coordinate-normalized-basis}
    \curl a = \begin{pmatrix}
      \frac{1}{r} \frac{\partial a^{3}}{\partial \phi} - \frac{\partial a^{2}}{\partial z} \\
      \frac{\partial a^{1}}{\partial z} - \frac{\partial a^{3}}{\partial r} \\
      \frac{\partial a^{2}}{\partial r} - \frac{1}{r} \frac{\partial a^{1}}{\partial \phi} +
      \frac{1}{r}a^2
    \end{pmatrix}.
  \end{equation}
...
\item Divergence of a vector $b$: $\divergence b$ is a scalar value which satisfies
  ...
  $\beta^2$ is
  \begin{equation}
    d\beta^2 = o(y) \left( \frac{\partial}{\partial y^1} \left( \sqrt{g(y)}b^1 \right) +
      \frac{\partial}{\partial y^2} \left( \sqrt{g(y)}b^2 \right) + \frac{\partial}{\partial y^3}
  ...
  \begin{equation}
    ...
    \divergence b = \frac{1}{\sqrt{g(y)}} \left( \frac{\partial}{\partial y^1} \left(
        \sqrt{g(y)}b^1 \right) + \frac{\partial}{\partial y^2} \left( \sqrt{g(y)}b^2 \right) +
      \frac{\partial}{\partial y^3} \left( \sqrt{g(y)}b^3 \right) \right).
  \end{equation}

  In the Cartesian coordinate chart,
  \begin{equation}
    \label{eq:div-expr-cartesian}
    \divergence b = \frac{\partial b^1}{\partial x^1} + \frac{\partial b^{2}}{\partial x^{2}} +
    \frac{\partial b^{3}}{\partial x^{3}}.
  \end{equation}

  In the ball coordinate chart $(r,\theta,\phi)$ with the non-normalized basis $\left\{
    \vect{\partial}r, \vect{\partial}\theta, \vect{\partial}\phi \right\}$,
  \begin{equation}
    \label{eq:div-expr-ball-coordinate-nonnormalized-basis}
    \divergence b = \frac{\partial b^{1}}{\partial r} + \frac{2}{r}b^1 + \frac{\partial
      b^{2}}{\partial \theta} + \frac{\cos\theta}{\sin\theta}b^2 + \frac{\partial b^{3}}{\partial
      \phi}. 
  \end{equation}
 If the normalized basis $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\theta},
   \vect{\partial}\hat{\phi} \right\}$ is adopted,
 \begin{equation}
   \label{eq:div-expr-ball-coordinate-normalized-basis}
   \divergence b = \frac{\partial b^{1}}{\partial r} + \frac{2}{r}b^1 +
   \frac{1}{r}\frac{\partial b^{2}}{\partial \theta} + \frac{\cos\theta}{r\sin\theta}b^2 +
   \frac{1}{r\sin\theta}\frac{\partial b^{3}}{\partial \phi}.
 \end{equation}
 ...
 \begin{equation}
   ...
   \divergence b = \frac{\partial b^{1}}{\partial r} + \frac{1}{r}b^1 + \frac{\partial
     b^{2}}{\partial \phi} + \frac{\partial b^{3}}{\partial z}.
 \end{equation}
 If the normalized basis $\left\{ \vect{\partial}\hat{r}, \vect{\partial}\hat{\phi},
   \vect{\partial}\hat{z} \right\}$ is adopted,
 \begin{equation}
   \label{eq:div-expr-cylindrival-coordinate-normalized-basis}
   \divergence b = \frac{\partial b^{1}}{\partial r} + \frac{1}{r}b^1 +
   \frac{1}{r}\frac{\partial b^{2}}{\partial \phi} + \frac{\partial b^{3}}{\partial z}.
 \end{equation}
\item Laplacian of a 0-form $f$: it is defined as the divergence of the gradient of $f$:
  ...
  \begin{equation}
    df = \frac{\partial f}{\partial y^{1}} dy^1 + \frac{\partial f}{\partial y^{2}} dy^2 +
    \frac{\partial f}{\partial y^{3}} dy^3.
  \end{equation}
  ...
  \begin{equation}
    ...
    \triangle f = \frac{1}{\sqrt{g(y)}} \left[ \frac{\partial}{\partial y^1} \left(
        \sqrt{g(y)} g^{1j} \frac{\partial f}{\partial y^{j}} \right) + \frac{\partial}{\partial
        y^2} \left( \sqrt{g(y)}g^{2j} \frac{\partial f}{\partial y^{j}} \right) +
      \frac{\partial}{\partial y^3} \left( \sqrt{g(y)} g^{3j} \frac{\partial f}{\partial y^{j}}
      \right) \right].
  \end{equation}
  ...
  \begin{equation}
    ...
    \triangle f = \frac{\partial^2 f}{\partial (x^1)^2} + \frac{\partial^2 f}{\partial (x^2)^2} +
    \frac{\partial^2 f}{\partial (x^3)^2}.
  \end{equation}
  ...
  \begin{equation}
    ...
    \triangle f = \frac{\partial^2 f}{\partial r^2} + \frac{2}{r} \frac{\partial f}{\partial r} +
    \frac{1}{r^2}\frac{\partial^2 f}{\partial \theta^2} +
    \frac{\cos\theta}{r^2\sin\theta}\frac{\partial f}{\partial \theta} +
    \frac{1}{r^2\sin\theta^2} \frac{\partial^2 f}{\partial \phi^2}.
  \end{equation}

  In the cylindrical coordinate chart $(r,\phi,z)$,
  \begin{equation}
    \label{eq:laplacian-expr-cylindrical-coordinate}
    \triangle f = \frac{\partial^2 f}{\partial r^2} + \frac{1}{r}\frac{\partial f}{\partial r} +
    \frac{1}{r^2} \frac{\partial^2 f}{\partial \phi^2} + \frac{\partial^2 f}{\partial z^2}.
  \end{equation}
\end{enumerate}
...
tangent vectors $\frac{\partial x}{\partial \hat{x}_1}$ and
$\frac{\partial x}{\partial \hat{x}_2}$ (see Figure \ref{fig:surface-element-scaling}).
...
\begin{equation}
  \tilde{\vect{e}}_3=\tilde{\vect{e}}_1\times\tilde{\vect{e}}_2=\left\vert\begin{matrix}
      ...
      \frac{\partial x_{1}}{\partial \hat{x}_{1}} & \frac{\partial x_{2}}{\partial \hat{x}_{1}} & \frac{\partial
        x_{3}}{\partial \hat{x}_{1}} \\
      \frac{\partial x_{1}}{\partial \hat{x}_{2}} & \frac{\partial x_{2}}{\partial \hat{x}_{2}} & \frac{\partial
        x_{3}}{\partial \hat{x}_{2}}
    \end{matrix}\right\vert=
  \left\vert\frac{\partial (x_{2},x_{3})}{\partial (\hat{x}_1,\hat{x}_2)}\right\vert \vect{i} +
  \left\vert\frac{\partial (x_{3},x_{1})}{\partial (\hat{x}_1,\hat{x}_2)}\right\vert \vect{j} +
  \left\vert\frac{\partial (x_{1},x_{2})}{\partial (\hat{x}_1,\hat{x}_2)}\right\vert \vect{k}
\end{equation}
...
\begin{mycomment}
  ...
  \begin{equation}
    ...
    \left\vert\begin{matrix}
        ...
        A_1 & A_2 & A_3
    \end{matrix}\right\vert = \left( \frac{\partial A_3}{\partial x_{\emphr{2}}} - \frac{\partial A_{2}}{\partial
      x_{\emphr{3}}} \right) \vect{i} + \left( \frac{\partial A_{1}}{\partial x_{\emphr{3}}} -
    \frac{\partial A_{3}}{\partial x_{\emphr{1}}} \right) \vect{j} + \left( \frac{\partial A_{2}}{\partial
      x_{\emphr{1}}} - \frac{\partial A_{1}}{\partial x_{\emphr{2}}} \right) \vect{k}.
  \end{equation}
...
Let $\abs{J_{23}} = \left\vert \frac{\partial (x_2,x_3)}{\partial (\hat{x}_1,\hat{x}_2)}\right\vert$,
$\abs{J_{31}} = \left\vert \frac{\partial (x_3,x_1)}{\partial (\hat{x}_1,\hat{x}_2)}\right\vert$ and
$\abs{J_{12}} = \left\vert \frac{\partial (x_1,x_2)}{\partial (\hat{x}_1,\hat{x}_2)}\right\vert$. Then
...
\section{Reason for making local copies of mapping objects in
  \texttt{sauter\_quadrature\_on\_one\_pair\_of\_dofs} and
  \texttt{sauter\_quadrature\_on\_one\_pair\_of\_cells}}
...
\item In the constructed DoF index-to-cell topology, \textbf{global} DoF indices in the
  ...
  \texttt{DoFHandler} are adopted. \emph{That's why such global DoF indices are passed
    into the function \texttt{sauter\_quadrature\_on\_one\_pair\_of\_dofs}.}
  \begin{lstlisting}[language=C++]
...
\item \texttt{i\_index} and \texttt{j\_index} calculated inside the function
  \texttt{sauter\_quadrature\_on\_one\_pair\_of\_dofs} are the array indices for accessing
...
\item $\overline{Z}_{\rm r}$ is the set of remaining row indices, which have not been selected or checked.
\item $\overline{Z}_{\rm c}$ is the set of remaining column indices, which have not been selected or checked.
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}[1]
    \Function{select\_ref\_row}{$A$, $\overline{Z}_{\rm r}$, $r$}
      \While {$\overline{Z}_{\rm r} \neq \Phi$}
        \State {Randomly select an index $p$ from $\overline{Z}_{\rm r}$.}
        \If {$p \neq r$}
          \State {Compute the $p$-th row of the matrix $A$: $v_p = a_{p,1:n}$.}
          \If {$\norm{v_p} \equiv 0$}
            \State {Remove the index $p$ from $\overline{Z}_{\rm r}$: $\overline{Z}_{\rm r} =
\overline{Z}_{\rm r}\backslash\{p\}$.}
            \State {Continue to select another non-zero row.}
          \Else
            \State {\Return $p$.}
          \EndIf
        \Else
          \If {$\abs{\overline{Z}_{\rm r}} \equiv 1$}
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}[1]
    \Function{select\_ref\_col}{$A$, $\overline{Z}_{\rm c}$, $c$}
      \While {$\overline{Z}_{\rm c} \neq \Phi$}
        \State {Randomly select an index $p$ from $\overline{Z}_{\rm c}$.}
        \If {$p \neq c$}
          \State {Compute the $p$-th column of the matrix $A$: $v_p = a_{1:m,p}$.}
          \If {$\norm{v_p} \equiv 0$}
            \State {Remove the index $p$ from $\overline{Z}_{\rm c}$: $\overline{Z}_{\rm c} =
\overline{Z}_{\rm c}\backslash\{p\}$.}
            \State {Continue to select another non-zero column.}
          \Else
            \State {\Return $p$.}
          \EndIf
        \Else
          \If {$\abs{\overline{Z}_{\rm c}} \equiv 1$}
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}[1]
    \State {Start with $k=1$, $\overline{Z}_{\rm r}=\{1:m\}$, $\overline{Z}_{\rm c}=\{1:n\}$,
$S=0$.}
    \State {Randomly select a reference row index $r$: $r=select\_ref\_row(A, \overline{Z}_{\rm
...
\overline{Z}_{\rm c}, -1)$.} \Comment{Initially, the current reference column index is $-1$.}
...
vector, hence there is a transposition here.}
      \State {Remove the selected row index from the set of remaining row indices: $\overline{Z}_{\rm
r}=\overline{Z}_{\rm r}\backslash\{i_k\}$}
      \State {Remove the selected column index from the set of remaining column indices:
$\overline{Z}_{\rm c}=\overline{Z}_{\rm c}\backslash\{j_k\}$}

      \If {$i_k \equiv r$} \Comment {If the current selected row clashes with the
reference row, reselect a reference.}
        \State {$r=select\_ref\_row(A, \overline{Z}_{\rm r}, r)$.}
        \State {Compute the $r$-th row of $A$ and store it as a column vector: $v_{\rm r}
= a_{r, 1:n}^T$.}
...
reference column, reselect a reference.}
        \State {$c=select\_ref\_col(A, \overline{Z}_{\rm c}, c)$.}
...
\frac{\varepsilon(1-\eta)}{1+\varepsilon}\norm{S_{k-1}}_{\rm F}$}
      ...
      \Else
        \State{$k=k+1$}
      \EndIf
    \Until{$\overline{Z}_{\rm r} \equiv \Phi$} \Comment{i.e. All rows of $A$ have been tried for selection.}
  \end{algorithmic}
\end{breakablealgorithm}
...
\section{Functional call hierarchy for assembling a full matrix}
Double loop over each pair of cells $K_x$ and $K_y$ is used to assemble a full matrix as
the Galerkin discretization of an integral operator. In the parallel version, TBB is used
to parallelize the inner loop over $K_y$. \textbf{CUDA is not used in either serial or
  parallel version.}

\subsection{Serial version}
\label{sec:orgb7c4d87}

\begin{codeitemize}
\item \texttt{assemble\_bem\_full\_matrix\_serial} \hfill \texttt{bem\_general.h}
  \begin{codeitemize}
  \item \texttt{sauter\_quadrature\_on\_one\_pair\_of\_cells} \hfill \texttt{sauter\_quadrature.hcu}
    \begin{codeitemize}
    \item \texttt{permute\_dofs\_and\_mapping\_support\_points\_for\_sauter\_quad} \hfill \texttt{sauter\_quadrature.hcu}
    \item \texttt{calc\_jacobian\_normals\_for\_sauter\_quad} \hfill \texttt{sauter\_quadrature.hcu}
      \begin{codeitemize}
      \item \texttt{surface\_jacobian\_det\_and\_normal\_vector} \hfill \texttt{bem\_tools.hcu}
      \item \texttt{transform\_quad\_point\_from\_unit\_to\_permuted\_real\_cell} \hfill \texttt{bem\_tools.hcu}
      \end{codeitemize}
    \item \texttt{calc\_covariant\_transformations} \hfill \texttt{sauter\_quadrature.hcu}
      \begin{codeitemize}
      \item \texttt{surface\_covariant\_transformation} \hfill \texttt{bem\_tools.hcu}
      \end{codeitemize}
    \item \texttt{ApplyQuadratureUsingBEMValues} \hfill \texttt{sauter\_quadrature.hcu}
    \end{codeitemize}
  \end{codeitemize}
\end{codeitemize}

\subsection{TBB parallel version}
\label{sec:orgf510e5f}

\begin{codeitemize}
\item \texttt{assemble\_bem\_full\_matrix} \hfill \texttt{bem\_general.h}
  \begin{codeitemize}
  \item \texttt{sauter\_quadrature\_on\_one\_pair\_of\_cells\_parallel\_over\_ky} \hfill \texttt{sauter\_quadrature.hcu}
    \begin{codeitemize}
    \item \texttt{sauter\_quadrature\_on\_one\_pair\_of\_cells} \hfill \texttt{sauter\_quadrature.hcu}
      \begin{codeitemize}
      \item \texttt{permute\_dofs\_and\_mapping\_support\_points\_for\_sauter\_quad} \hfill \texttt{sauter\_quadrature.hcu}
...
\section{Functional call hierarchy for assembling an $\mathcal{H}$-matrix}

Generally, there are three overloaded functions for building an \(\mathcal{H}\)-matrix
related to an boundary integral operator:
\begin{enumerate}
\item \(\mathcal{H}\)-matrix, e.g. matrix $\mathcal{V}$ for the single layer potential operator $V$.
\item \(\mathcal{H}\)-matrix plus a mass matrix, e.g. matrix
  $\frac{1}{2}\mathcal{M} + \mathcal{K}$ related to the double layer potential operator
  $K$ plus the identity operator $\frac{1}{2}I$.
\item \(\mathcal{H}\)-matrix plus a stabilization or regularization term, e.g. matrix
  $\widetilde{\mathcal{D}}$ for the modified bilinear form associated with the
  hypersingular operator $D$:
  \begin{equation}
    \langle \widetilde{D}u, v \rangle = \langle Du,v \rangle + \alpha \langle u,1
     \rangle\cdot \langle v,1 \rangle,
  \end{equation}
  or
  \begin{equation}
    \langle \widetilde{D}u, v \rangle = \langle Du,v \rangle + \alpha \langle u,w_{\mathrm{eq}}
    \rangle\cdot \langle v,w_{\mathrm{eq}} \rangle.
  \end{equation}
\end{enumerate}
Because the mass matrix $\mathcal{M}$ above only contributes to near field
$\mathcal{H}$-matrix blocks, when we build a far field $\mathcal{H}$-matrix block, there
are only the two overloaded versions 1 and 3.

\subsection{Serial on CPU + CUDA version}
\label{sec:org4e26621}
The function runs in serial mode on CPU without using TBB. For Sauter quadrature during
the assembly of both near field and far field \(\mathcal{H}\)-matrix blocks, iterations
are performed over each pair of cells in the support sets of each pair of DoFs. For each
of such Sauter quadrature, the $k_3$ terms and quadrature points are distributed to GPU
threads. Obviously, because Sauter quadrature tasks are not collected for batch
processing, the GPU parallelization efficiency in this implementation is low.

The function call hierarchy is below.
\begin{codeitemize}
\item \texttt{fill\_hmatrix\_with\_aca\_plus\_serial} (3 versions) \hfill
  \texttt{aca\_plus.hcu}
  \begin{codeitemize}
  \item \texttt{fill\_hmatrix\_leaf\_node\_with\_aca\_plus} (3 versions) \hfill
    \texttt{aca\_plus.hcu}
    \begin{codeitemize}
    \item \texttt{sauter\_quadrature\_on\_one\_pair\_of\_dofs} (2 versions) \hfill
      \texttt{sauter\_quadrature.hcu}
      \begin{codeitemize}
      \item \texttt{permute\_dofs\_and\_mapping\_support\_points\_for\_sauter\_quad}
        \hfill \texttt{sauter\_quadrature.hcu}
      \item
        \texttt{CUDAWrappers::calc\_jacobian\_normals\_for\_sauter\_quad\_\{neighboring\_type\}}
        \hfill \texttt{sauter\_quadrature.hcu}
        
        \emph{N.B. To reduce the cost on branching, for each cell neighboring type, a
          kernel function is defined. The suffix \texttt{\{neighboring\_type\}} in the
          function name can be: \texttt{same\_panel}, \texttt{common\_edge},
          \texttt{common\_vertex}, \texttt{regular}.}
        \begin{codeitemize}
        \item \texttt{BEMTools::CUDAWrappers::surface\_jacobian\_det\_and\_normal\_vector}
          \hfill \texttt{bem\_tools.hcu}
        \item
          \texttt{BEMTools::CUDAWrappers::transform\_quad\_point\_from\_unit\_to\_permuted\_real\_cell}
          \hfill \texttt{bem\_tools.hcu}
        \end{codeitemize}
      \item \texttt{CUDAWrappers::calc\_covariant\_transformations\_\{neighboring\_type\}}
        \hfill \texttt{sauter\_quadrature.hcu}
        \begin{codeitemize}
        \item \texttt{BEMTools::CUDAWrappers::surface\_covariant\_transformation} \hfill
          \texttt{bem\_tools.hcu}
        \end{codeitemize}
      \item \texttt{CUDAWrappers::ApplyQuadratureUsingBEMValues\{NeighboringType\}} \hfill
        \texttt{sauter\_quadrature.hcu}
      \end{codeitemize}
    \item \texttt{aca\_plus\_parallel\_sauter\_quad\_for\_vector\_entry} (2 versions) \hfill \texttt{aca\_plus.hcu}
      \begin{codeitemize}
      \item \texttt{random\_select\_ref\_row(Vector, ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
        \begin{codeitemize}
        \item \texttt{assemble\_kernel\_row(Vector, ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
          \begin{codeitemize}
          \item \texttt{sauter\_quadrature\_on\_one\_pair\_of\_far\_field\_dofs} \hfill
            \texttt{sauter\_quadrature.hcu}
            
            \emph{The following functions calls are the same to those under
            \texttt{sauter\_quadrature\_on\_one\_pair\_of\_dofs} which is called by
            \texttt{fill\_hmatrix\_with\_aca\_plus\_serial}.}
            \begin{codeitemize}
            \item \texttt{permute\_dofs\_and\_mapping\_support\_points\_for\_sauter\_quad}
              \hfill \texttt{sauter\_quadrature.hcu}
            \item
              \texttt{CUDAWrappers::calc\_jacobian\_normals\_for\_sauter\_quad\_regular}
              \hfill \texttt{sauter\_quadrature.hcu}
              \begin{codeitemize}
              \item
                \texttt{BEMTools::CUDAWrappers::surface\_jacobian\_det\_and\_normal\_vector}
                \hfill \texttt{bem\_tools.hcu}
              \item
                \texttt{BEMTools::CUDAWrappers::transform\_quad\_point\_from\_unit\_to\_permuted\_real\_cell}
                \hfill \texttt{bem\_tools.hcu}
              \end{codeitemize}
            \item \texttt{CUDAWrappers::calc\_covariant\_transformations\_regular} \hfill
              \texttt{sauter\_quadrature.hcu}
              \begin{codeitemize}
              \item \texttt{BEMTools::CUDAWrappers::surface\_covariant\_transformation}
                \hfill \texttt{bem\_tools.hcu}
              \end{codeitemize}
            \item \texttt{CUDAWrappers::ApplyQuadratureUsingBEMValuesRegular} \hfill
              \texttt{sauter\_quadrature.hcu}
            \end{codeitemize}
          \end{codeitemize}
        \end{codeitemize}
      \item \texttt{random\_select\_ref\_column(Vector, ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
        \begin{codeitemize}
        \item \texttt{assemble\_kernel\_column(Vector, ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
          \begin{codeitemize}
          \item \texttt{sauter\_quadrature\_on\_one\_pair\_of\_far\_field\_dofs} \hfill
            \texttt{sauter\_quadrature.hcu}
            \begin{codeitemize}
...
\subsection{TBB parallel + CUDA version}
\label{sec:orgb2752cf}

TBB is used to partition the near field leaf set of the $\mathcal{H}$-matrix into several
ranges. Producer-consumer model is used for the assembly of near field
\(\mathcal{H}\)-matrix blocks, during which Sauter quadrature tasks for different cell
neighboring types are collected into their respective ring buffers and then sent to GPU
for batch processing.

For far field \(\mathcal{H}\)-matrix block assembly, a task buffer is adopted for
assembling a whole row or whole column vector in the low-rank matrix. The computation of
each vector entry is parallelized on GPU.

\begin{codeitemize}
\item \texttt{fill\_hmatrix\_with\_aca\_plus\_smp} (3 versions) \hfill
  \texttt{aca\_plus.hcu}
  \begin{codeitemize}
  \item For near field \(\mathcal{H}\)-matrix blocks
    \begin{codeitemize}
    \item \texttt{sauter\_quadrature\_for\_near\_field\_matrix\_entries\_producer} (2
      versions) \hfill \texttt{sauter\_quadrature.hcu}
      \begin{codeitemize}
      \item \texttt{create\_sauter\_quadrature\_tasks\_on\_one\_pair\_of\_dofs} (2
        versions) \hfill \texttt{sauter\_quadrature.hcu}
        \begin{codeitemize}
        \item \texttt{permute\_dofs\_and\_mapping\_support\_points\_for\_sauter\_quad}
          \hfill \texttt{sauter\_quadrature.hcu}
        \item \texttt{SauterQuadratureTaskRingBuffer::add\_task} \hfill
          \texttt{sauter\_quadrature.hcu}
        \end{codeitemize}
      \end{codeitemize}
    \item \texttt{sauter\_quadrature\_for\_near\_field\_matrix\_entries\_consumer} \hfill
      \texttt{sauter\_quadrature.hcu}
      \begin{codeitemize}
      \item \texttt{SauterQuadratureTaskScalarData::fetch\_and\_process\_task\_batch}
        \hfill \texttt{sauter\_quadrature.hcu}
        \begin{codeitemize}
        \item
          \texttt{process\_sauter\_task\_on\_one\_pair\_of\_dofs\_\{neighboring\_type\}}
          \hfill \texttt{sauter\_quadrature.hcu}
          \begin{codeitemize}
          \item
            \texttt{BEMTools::CUDAWrappers::surface\_jacobian\_det\_normal\_vector\_and\_covariant}
            \hfill \texttt{bem\_tools.hcu}
          \item
            \texttt{BEMTools::CUDAWrappers::transform\_quad\_point\_from\_unit\_to\_permuted\_real\_cell}
            \hfill \texttt{bem\_tools.hcu}
      ...
      \end{codeitemize}
    \end{codeitemize}
  \item For far field \(\mathcal{H}\)-matrix blocks
    \begin{codeitemize}
    \item \texttt{fill\_far\_field\_hmatrix\_leaf\_node\_subrange\_with\_aca\_plus} (2
      versions) \hfill \texttt{aca\_plus.hcu}
      \begin{codeitemize}
      \item \texttt{fill\_far\_field\_hmatrix\_leaf\_node\_with\_aca\_plus} (2 versions)
        \hfill \texttt{aca\_plus.hcu}
        \begin{codeitemize}
        \item \texttt{aca\_plus\_parallel\_sauter\_quad\_for\_whole\_vector} (2 versions)
          \hfill \texttt{aca\_plus.hcu}
          \begin{codeitemize}
          \item \texttt{random\_select\_ref\_row(SauterQuadratureTaskBufferForVector,
              ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
            \begin{codeitemize}
            \item \texttt{assemble\_kernel\_row(SauterQuadratureTaskBufferForVector, ...)}
              (2 versions) \hfill \texttt{aca\_plus.hcu}
              \begin{codeitemize}
              \item \texttt{create\_sauter\_quadrature\_tasks\_on\_one\_pair\_of\_dofs}
                \hfill \texttt{sauter\_quadrature.hcu}
                \begin{codeitemize}
                \item
                  \texttt{permute\_dofs\_and\_mapping\_support\_points\_for\_sauter\_quad}
                  \hfill \texttt{sauter\_quadrature.hcu}
                \end{codeitemize}
              \item \texttt{SauterQuadratureTaskBufferForVector::process\_tasks} \hfill
                \texttt{sauter\_quadrature.hcu}
                \begin{codeitemize}
                \item
                  \texttt{process\_sauter\_tasks\_in\_buffer\_for\_far\_field\_hmat\_vector}
                  \hfill \texttt{sauter\_quadrature.hcu}
                  \begin{codeitemize}
                  \item
                    \texttt{BEMTools::CUDAWrappers::surface\_jacobian\_det\_normal\_vector\_and\_covariant}
                    \hfill \texttt{bem\_tools.hcu}
                  \item
                    \texttt{\small BEMTools::CUDAWrappers::transform\_quad\_point\_from\_unit\_to\_permuted\_real\_cell}
                    \hfill \texttt{bem\_tools.hcu}
                  \end{codeitemize}
                \end{codeitemize}
              \end{codeitemize}
            \end{codeitemize}
          \item \texttt{random\_select\_ref\_column(SauterQuadratureTaskBufferForVector,
              ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
            \begin{codeitemize}
            \item \texttt{assemble\_kernel\_column(SauterQuadratureTaskBufferForVector,
                ...)} (2 versions) \hfill \texttt{aca\_plus.hcu}
              \begin{codeitemize}
              \item \texttt{create\_sauter\_quadrature\_tasks\_on\_one\_pair\_of\_dofs}
                \hfill \texttt{sauter\_quadrature.hcu}
                ...
                \item
                  ...
                  \hfill \texttt{sauter\_quadrature.hcu}
                \end{codeitemize}
              \item \texttt{SauterQuadratureTaskBufferForVector::process\_tasks}
                \texttt{sauter\_quadrature.hcu}
                \begin{codeitemize}
                \item
                  \texttt{process\_sauter\_tasks\_in\_buffer\_for\_far\_field\_hmat\_vector}
...
\begin{equation}
  \frac{\partial u}{\partial n}\Big\vert_{\Gamma} = n(x)\cdot\nabla_x
...
\begin{equation}
  \frac{\partial u}{\partial n}\Big\vert_{\Gamma} = \frac{1}{4\pi}\frac{(x,
    x_0-x)}{\norm{x-x_0}^3} \quad (x\in\Gamma).
\end{equation}
...
\item Neumann problem
  \begin{equation}
    \begin{cases}
      \triangle u = 0 & \text{in $\Omega^{\rm c}\coloneqq\mathbb{R}^3\backslash\overline{\Omega}$} \\
      \frac{\partial u}{\partial n}\Big\vert_{\Gamma} = g_{\rm N} = \frac{1}{4\pi}\frac{(n(x),
...
\begin{equation}
  \int_{\Gamma} -\frac{\partial u}{\partial n} \intd s = 1.
\end{equation}
