...
\section{Manifold and tangent vector}
...
time $\intd t$ is actually walking along a straight line $x_U(t) = p_U+t\vect{v}_U$. Hence
...
\subsubsection{Tangent vector as a differential operator}
...
as the basis vector $\vect{e}_j$ of the Cartesian frame $\mathbb{R}^d$, i.e. the $i$-th
component of $\vect{\pdiff}x^j$ is the Kronecker $\delta_j^i$. However, it is different
from $\vect{e}_j$ in the global Cartesian frame. It is actually the tangent vector or
velocity vector of the $j$-th coordinate curve at $p$ in $(U,x)$.
\emph{$\left\{ \vect{\pdiff} x^j \right\}_{j=1}^d$ can be considered as a local Cartesian
  coordinate frame at $p$ using the coordinate chart $(U,x)$.}
...
$\left\{\frac{\pdiff}{\pdiff x^j}\Big\vert_p\right\}_{j=1}^d$, which is written simply as
$\left\{\frac{\pdiff}{\pdiff x^j} \right\}_{j=1}^d$. This is the same as the results
derived from treating a tangent vector as a differential operator.
...
via the Jacobi matrix of the transition map $\phi_{VU}$, since each basis vector is a
tangent vector, which follows the same transformation rule as in
