...
the Jacobi matrix. Hence, $F_{*}$ here is just the Jacobi map. Then the push-back of the
...
\begin{Definition}[Pull-back of a $k$-form]
  ...
  \end{equation}
  where $F_{*}$ is the Jacobi map from $W_p^m$ to $M_p^n$.
\end{Definition}
...
\begin{Proof}
  ...
  \begin{align*}
    ...
    \intertext{Apply Equation (\ref{eq:cotangent-base-appled-to-tangent-base})}
    &= \sum_I \left( \frac{\pdiff y^{j_1}}{\pdiff x^{i_1}} \cdots \frac{\pdiff
...
\begin{Example}[Pull-back of the inclusion map]
  ...
  the base $\left\{ \vect{\pdiff}x^1, \vect{\pdiff}x^2, \vect{\pdiff}x^3 \right\}$ of the
  ...
  $\left\{ \vect{\pdiff}u^1,\vect{\pdiff}u^2 \right\}$ may not be an orthonormal base,
  ...
  columns of the Jacobi map $\vect{i}_{*}$ in its matrix form. Then the normal vector is
  \begin{equation*}
