...
\begin{Example}[Sphere ($S^2$) as a two dimensional submanifold in $\mathbb{R}^3$]
  ...
  $\frac{\pdiff z}{\pdiff y}$ will be finite. Therefore, the upper hemisphere excluding
...
\begin{Definition}[Tangent vector]
  A tangent vector $v$ at a point $p$ in an $n$-dimensional manifold $M^n$ is
  defined as an entity that for each coordinate chart $(U,x)$ containing $p$, it can be
...
\begin{Definition}[Directional derivative of a function with respect to a vector]
  ...
  \begin{equation}
    ...
    D_{\vect{v}}(f) = \frac{\diff}{\diff t} \left[ f(p + t\vect{v}) \right] \big\vert_{t=0}.
  \end{equation}
\end{Definition}
...
Assume $f$ is defined on a $d$-dimensional manifold with a coordinate chart $(U, x_{U})$
containing $p$. Since a manifold is locally homeomorphic to $\mathbb{R}^d$, starting from
$p$ then moving with the speed vector $\vect{v}_U$ on the manifold in an infinitesimal
time $\diff t$ is actually walking along a straight line $x_U(t) = p_U+t\vect{v}_U$. Hence
the above derivative of $f$ can be expanded as
\begin{equation}
  \label{eq:df-in-xu}
  D_{\vect{v}_U}(f) = \sum_{i=1}^d \frac{\pdiff f}{\pdiff x_U^i} \frac{\diff
    x_{U}^i(t)}{\diff t} = \sum_{i=1}^d \frac{\pdiff f}{\pdiff x_U^i}\Bigg\vert_{x_U=p_U} v_U^i.
\end{equation}
If another coordinate chart $(V, x_V)$ is adopted which overlaps with $(U,x_U)$, the
derivative of $f$ can also be written as
\begin{equation}
  \label{eq:df-in-xv}
  D_{\vect{v}_V}(f) = \sum_{i=1}^d \frac{\pdiff f}{\pdiff x_V^i} \frac{\diff
    x_{V}^i(t)}{\diff t} = \sum_{i=1}^d \frac{\pdiff f}{\pdiff x_V^i} \Bigg\vert_{x_V=p_V} v_V^i.
\end{equation}
...
\begin{Proof}[Directional derivative of $f$ is coordinate independent]
  ...
  \begin{equation}
    ...
    v_V^i = \sum_{j=1}^d \frac{\pdiff x_{V}^i}{\pdiff x_{U}^j} x_U^j \quad (i=1,\cdots,d).
  \end{equation}
  Substitute this transformation into Equation (\ref{eq:df-in-xv}), we have
  $$
  D_{\vect{v}_V}(f) = \sum_{i=1}^d \frac{\pdiff f}{\pdiff x_V^i} \sum_{j=1}^d \frac{\pdiff
    x_{V}^i}{\pdiff x_{U}^j} x_U^j = \sum_{j=1}^d \left( \sum_{i=1}^d \frac{\pdiff f}{\pdiff
      x_V^i} \frac{\pdiff x_{V}^i}{\pdiff x_{U}^j} \right) x_U^j = \sum_{j=1}^d \frac{\pdiff
...
\begin{Remark}
  ...
  $x_V$. And it is neither necessarily the same as the manifold dimension $d$. This is
...
a tangent vector $X_p$ rooted at $p$ in a local coordinate chart $(U,x)$ can be
identified with a differential operator:
\begin{equation}
  \label{eq:tangent-vector-as-diff-op}
  X_p=\sum_jX^j \frac{\pdiff}{\pdiff x^j}\Bigg\vert_p.
\end{equation}
Here $X_j$ is the vector coefficient and the differential operator
...
simpler form $\vect{\pdiff} x^j$. Hence, $\left\{ \vect{\pdiff} x^j \right\}_{j=1}^d$ is a
basis of the coordinate chart $(U,x)$. Since the $j$-th component of $X_p$ in this chart
is just $X^j$, the corresponding basis vector $\vect{\pdiff} x^j$ is numerically the same
as the basis vector $\boldsymbol{e}_j$ of the Cartesian frame $\mathbb{R}^d$, i.e. the $i$-th
...
\begin{Definition}[Coordinate curve]
  The $i$-th coordinate curve of a coordinate chart $(U,x)$ with $x=(x^1,\cdots,x^d)$ is
  defined as
  \begin{equation}
...
\begin{equation}
  ...
  \frac{\diff x}{\diff t}\Bigg\vert_{t=0}=(\delta_{j}^1, \cdots, \delta_j^{d}).
\end{equation}
...
$\left\{ \vect{\pdiff} x^j \right\}_{j=1}^d$. This is the same as the results derived from
treating a tangent vector as a differential operator.

If a different coordinate chart $(V,y)$ containing $p$ is adopted, where
$y = (y^1,\cdots,y^{d'})$ may have different number of components from $x$, the basis
$\left\{ \vect{\pdiff}x^j \right\}_{j=1}^d$ in $(U,x)$ can be transformed to the new chart
...
\begin{equation}
  ...
  \sum_{k=1}^d \frac{\pdiff y^i}{\pdiff x^k} (\vect{\pdiff} x^j)_k = \sum_{k=1}^d
  \frac{\pdiff y^i}{\pdiff x^k} \delta_k^j = \frac{\pdiff y^{i}}{\pdiff x^{j}} \quad (i=1,\cdots,d').
\end{equation}
...
\subsection{Coordinate transformation of a basis vector in the tangent space}

Assume there are two coordinate charts $(U,x_U)$ and $(V,x_V)$ assigned to the tangent
space $M_p^n$. The basis vector of $M_p^n$ represented in $(U,x_U)$ is
$\left\{ \vect{\pdiff}x_U^j \right\}$ and the basis vector represented in $(V,x_V)$ is
$\left\{ \vect{\pdiff}x_V^i \right\}$. Then we want to know how each basis vector
$\vect{\pdiff}x_V^i$ is expanded in the basis $\left\{ \vect{\pdiff}x_U^j \right\}$, i.e.
we want to compute the expansion coefficients $a_j$ in the following expression
\begin{equation*}
...
equivalent to $\vect{e}_i$, the above expression extracts the $i$-th column of the Jacobi
matrix. Therefore, $a^{j}$ is just the $j$-th component in this column vector, i.e.
\begin{equation}
  a^j = \frac{\pdiff x_U^j}{\pdiff x_V^i}.
\end{equation}
Hence, the basis vector $\vect{\pdiff}x_V^i$ is represented as
\begin{equation}
  \label{eq:tangent-vector-basis-transformation}
...
$(\vect{\pdiff}x_U^1,\cdots,\vect{\pdiff}x_U^n)$, the matrix formulation of the contravariant
basis transformation is
\begin{equation}
...
\end{Definition}
The inner product in matrix formulation is
\begin{equation}
...
means the tangent vector $a$ determines a covector $\alpha$.

\begin{Theorem}[Riesz representation theorem]
  Let $X$ be a Hilbert space and $f$ be a bounded linear functional on $X$, i.e. $f \in
  X'$. There exists a unique vector $a$ in $X$, such that for all $b$ in $X$, $f(b) =
  \left\langle a, b \right\rangle$ and $\norm{f} = \norm{a}$.
\end{Theorem}
...
\end{equation}
Here $G^{-1}$ is an another tensor, whose entry is written as $g^{ij}$. Therefore
\begin{equation}
...
\subsection{Coordinate transformation of the metric tensor}

According to Definition \ref{def:metric-tensor}, the metric tensor depends on the basis
vectors of the tangent space. Therefore, using the coordinate transformation of
tangent basis vectors in Equation (\ref{eq:tangent-vector-basis-transformation}), we can
obtain the coordinate transformation of a metric tensor.
...
\begin{equation*}
  g_{rs}^V = \left\langle \vect{\pdiff}x_U^r, \vect{\pdiff}x_U^s \right\rangle
\end{equation*}
in the chart $(U,x_U)$.

According to Equation (\ref{eq:tangent-vector-basis-transformation}), we have
\begin{equation*}
