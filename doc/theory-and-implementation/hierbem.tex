...
$\left( \otimes^r M_p^n \right)^*r$, which is a Cartesian product of $r$ cotangent spaces
$M_p^{n*}$,
\begin{equation}
...
\begin{Definition}[Covariant tensor]
  ...
  tangent spaces $\otimes^r M_p^n$, i.e. $Q: \otimes^r M_p^n \rightarrow \mathbb{R}$.
\end{Definition}
...
\begin{equation}
  ...
  Q(v_1,\cdots,v_r) = v_1^{i_1}\cdots v_r^{i_r} Q(\vect{\pdiff}x^{i_1},\cdots,\vect{\pdiff}x^{i_r}).
\end{equation}

An $r$-th rank covariant tensor $Q$ can also be considered as a \emph{tensor product} of
$r$ 1-forms. Let $\alpha^1,\cdots,\alpha^r$ be 1-forms in $M_p^{n*}$, their tensor product
...
\end{equation}
Then expand $\prod_i \alpha^i(v_i)$:
\begin{equation}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
This means the construction of a $r$-rank covariant tensor via tensor product of $r$
...
the component of a tensor.
 ...
 component of the inverse of the metric tensor contravariant tensor: e.g. $g^{ji}$, the inverse of the metric
    tensor that transforms a covector to a contravariant vector,
    i.e. the subscript is promoted to a superscript.
