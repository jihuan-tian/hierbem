...
namespace IdeoBEM
{
  ...
  namespace LaplaceKernel
  {
    ...
    class HyperSingularKernelRegular
      : public KernelFunction<dim, RangeNumberType>
    {
    public:
       ...
       * the hyper-singular kernel, the value calculated here is actually not
       * the hyper-singular function itself, but the fundamental solution of the
       * Laplace operator. The final computing the regularized bilinear form
       * will be carried out in the pullback in the unit cell, which is handled
       * in the
