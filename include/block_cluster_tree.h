...
namespace HierBEM
{
             ...
             const BlockClusterTree<spacedim, Number> &block_cluster_tree)
  {
    ...
    out << "* Near field set: "
        << block_cluster_tree.get_near_field_set().size()
        << " block clusters\n";
                ...
                typename BlockClusterTree<spacedim, Number>::node_pointer_type
                  child_block_cluster_node = CreateTreeNode<
                    ...
                    typename BlockClusterTree<spacedim, Number>::
                      data_value_type(tau_son_node_pointer, sigma_node_pointer),
                    bc_node->get_level() + 1,
                 ...
                 i++)
              {
                ...
                typename BlockClusterTree<spacedim, Number>::node_pointer_type
                  child_block_cluster_node = CreateTreeNode<
                    ...
                    BlockClusterTree<spacedim, Number>::child_num>(
                    typename BlockClusterTree<spacedim, Number>::
                      data_value_type(tau_node_pointer, sigma_son_node_pointer),
                    bc_node->get_level() + 1,
                    empty_child_pointers,
                    bc_node,
                    UnsplitMode);
                     ...
                     j++)
                  {
                    typename ClusterTree<spacedim, Number>::node_pointer_type
                      sigma_son_node_pointer = bc_node->get_data_pointer()
                                                 ->get_sigma_node()
                                                 ->get_child_pointer(j);
                    Assert(sigma_son_node_pointer != nullptr,
                           ExcInternalError());
                 ...
                 j++)
              {
                ...
                if (i == j)
                ...
                else
                  {
                     ...
                     */
                    child_block_cluster_node->get_data_pointer()->is_small(
                      n_min);
    ...
    std::vector<node_pointer_type>     &leaf_set_wrt_current_node)
  {
    ...
    if (current_block_cluster_node->get_data_pointer()->is_admissible_or_small(
    ...
    else
      {
         ...
         * </dl>
         */
        Assert(current_block_cluster_node->get_child_num() ==
                 (BlockClusterTree<spacedim, Number>::child_num),
      ...
      *current_bc_node)
  {
    ...
    out << "\"" << std::hex << current_bc_node << "\""
        << "[label=<<b>" << std::hex << current_bc_node << "</b><br/>"
        << std::dec << "tau: [";
