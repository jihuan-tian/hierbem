...
public:
  ...
  /**
   * Construct from two cluster nodes, whose Cartesian product is the root node
   * of the block cluster tree.
   *
   * After construction, there is only one node in the block cluster tree,
   * i.e. tree hierarchy is not built.
   *
   ...
   * new tree but never a subtree of an existing block cluster tree. Hence, the
  ...
  /**
   * Clear the data field of the block cluster tree because its memory has been
   * migrated to another object via shallow copy.
   */
  void
  clear();
   ...
   * <dl class="section note">
   *   <dt>Note</dt>
   *   <dd><strong>This algorithm iterates over each element in the leaf set of
   * the block cluster tree to be extended. During the iteration, because leaf
   * nodes may be further split into smaller ones, the leaf set is not a
   ...
   * leaf set of of which will be built after the whole tree hierarchy has been
   * constructed.</strong>
   *   </dd>
   * </dl>
   * @param P
 ...
 * split mode.
 *
 * After the splitting, the total number of block cluster nodes in the \bct will
 * be updated. Whether the leaf set will be added with the newly created \bc
 * nodes is controlled by the flag \p if_add_child_nodes_to_leaf_set.
 *
 * \mynote{If the newly created \bc nodes are to be added to the leaf set, their
               ...
               i++)
            {
              ...
              for (unsigned int j = 0;
                   ...
                   j++)
                {
                   ...
                   * \in S(\tau)\f$ and \f$\sigma^* \in S(\sigma)\f$. Its parent
                   * node is set to the current block cluster node.
                   */
                  typename BlockClusterTree<spacedim, Number>::node_pointer_type
                    child_block_cluster_node = CreateTreeNode<
...
/**
 * Prune all those \bc nodes which are descendants of the \bc nodes specified in
...
operator=(const BlockClusterTree<spacedim, Number> &bct)
{
   ...
   * tree, clear its data members instead of releasing their memory.
   */
  if (is_subtree)
    {
      clear();
...
{
  /**
   * Iterate over the leaf set of the current block cluster tree and look for a
   * block cluster which is not contained by any block cluster in the given
   * partition.
   */
   ...
   * actually been extended. If true, the leaf set of the current block cluster
   * tree has been modified during the extension, which needs a further
  ...
  while (node_pointer_type current_block_cluster_node =
    ...
    {
      /**
       * Because the selected block cluster node in the leaf set is about to be
       * split, we delete the current block cluster node from the leaf set
      ...
      /**
       * Select a block cluster node in the given partition, whose index set has
       * a nonempty intersection with the index set of the current block cluster
       * node.
       */
      ...
      /**
       * Because the given partition is a covering of the complete block cluster
       * index set \f$I \times J\f$, such block cluster node must exist. Hence
      ...
      if (current_block_cluster_node->get_data_reference()
                                  ...
                                  ->get_data_reference()))
        {
          if (current_block_cluster_node->get_data_reference()
          ...
          else
            {
              /**
               * Extend the current block cluster node by splitting its
               * \f$\tau\f$ node, which is horizontal split, then make Cartesian
               * product with its \f$\sigma\f$ node. The newly created \bc nodes
               * are appended to the leaf set.
               */
              split_block_cluster_node(current_block_cluster_node,
                                       *this,
                                       HorizontalSplitMode,
        ...
        {
          /**
           * Extend the current block cluster node by splitting its \f$\sigma\f$
           * node, which is vertical split, then make Cartesian product with its
           * \f$\tau\f$ node. The newly created \bc nodes are appended to the
           * leaf set.
           */
          split_block_cluster_node(current_block_cluster_node,
                                   *this,
  ...
  const std::vector<node_pointer_type> &partition)
{
  /**
   * Iterate over the leaf set of the current block cluster tree and look for a
   * block cluster which does not belong to the partition.
   */
  // Iterator pointing to the selected block cluster node in the leaf set.
  typename std::vector<node_pointer_type>::const_iterator
    it_for_desired_bc_node;
  ...
  // extended, thus the leaf set is also modified and needs a re-categorization
  // into the near field set and the far field set.
  bool is_tree_extended = false;
  ...
  while (
      ...
      find_leaf_bc_node_not_in_partition(partition, it_for_desired_bc_node))
    {
      ...
      /**
       * Select a block cluster node in the given partition, whose index set is
       * a proper subset of the index set of the current block cluster node.
       */
      node_pointer_type bc_node_in_partition =
        find_bc_node_in_partition_proper_subset_of_current_bc_node(
          current_block_cluster_node, partition);
      ...
      /**
       * Because the partition is a covering of the complete block cluster index
       * set \f$I \times J\f$ and it is finer than the leaf set of the current
       * block cluster tree, such block cluster node must exist. Hence we make
       * an assertion here.
       */
      Assert(bc_node_in_partition != nullptr, ExcInternalError());
                                  ...
                                  ->get_data_reference()))
        {
          if (current_block_cluster_node->get_data_reference()
          ...
          else
            {
               ...
               * Extend the current block cluster node by splitting its
               * \f$\tau\f$ node, which is horizontal split, then make Cartesian
               * product with its \f$\sigma\f$ node. The newly
               * created \bc nodes are appended to the leaf set.
               */
              split_block_cluster_node(current_block_cluster_node,
                                       *this,
           ...
           i++)
        {
          ...
          for (unsigned int j = 0;
               ...
               j++)
            {
               ...
               * Create a new block cluster node as child. Then append this new
               * node as one of the children of the current block cluster node.
               * Finally, recursively partition from this node using the fine
               * non-tensor product method if the two component clusters have
               * the same indices, i.e. \f$I_1 \times I_1\f$ and \f$I_2 \times
               * I_2\f$; for \f$I_1 \times I_2\f$, recursively partition from it
               * using the \f$\mathcal{N}\f$-type partition method; for \f$I_2
               * \times I_1\f$, recursively partition from it using the
               * \f$\mathcal{N}^*\f$-type partition method.
               */
              const std::array<node_pointer_type,
                               BlockClusterTree<spacedim, Number>::child_num>
                                empty_child_pointers{nullptr, nullptr, nullptr, nullptr};
              ...
              if (i == j)
                {
                  /**
                   * Handle the case for \f$I_1 \times I_1\f$ and \f$I_2 \times
                   * I_2\f$ by recursively applying the fine non-tensor product
                   * partition method itself.
                   */
                  std::vector<node_pointer_type> leaf_set_wrt_child_node;
                  partition_fine_non_tensor_product_from_block_cluster_node(
                    child_block_cluster_node, leaf_set_wrt_child_node);
           ...
           i++)
        {
          ...
          for (unsigned int j = 0;
               ...
               j++)
            {
               ...
               * Create a new block cluster node as child. Then append this new
               * node as one of the children of the current block cluster node.
               * Finally, recursively partition from this node if it is on the
               * bottom left corner, i.e. it is the \f$I_2 \times I_1\f$ block
               * cluster; otherwise, for \f$I_1 \times I_1\f$, \f$I_1 \times
               * I_2\f$ and \f$I_2 \times I_2\f$, stop the recursion and
               * directly add them to the leaf set.
               */
              const std::array<node_pointer_type,
                               BlockClusterTree<spacedim, Number>::child_num>
           ...
           i++)
        {
          ...
          for (unsigned int j = 0;
               ...
               j++)
            {
               ...
               * node as one of the children of the current block cluster node.
               * Finally, recursively partition from this node if it is on the
               * top right corner, i.e. it is the \f$I_1 \times I_2\f$ block
               * cluster; otherwise, for \f$I_1 \times I_1\f$, \f$I_2 \times
               * I_1\f$ and \f$I_2 \times I_2\f$, stop the recursion and
               * directly add them to the leaf set.
               */
              const std::array<node_pointer_type,
                               BlockClusterTree<spacedim, Number>::child_num>
  ...
  for (it_for_desired_bc_node = leaf_set.cbegin();
       ...
       it_for_desired_bc_node++)
    {
      ...
      for (node_pointer_type bc_node_in_partition : partition)
        {
          if (bc_node_in_leaf_set->get_data_reference().is_subset(
                bc_node_in_partition->get_data_reference()))
            {
               ...
               * node in the given partition, terminate the inner loop and jump
               * to the next leaf node for checking.
               */
      ...
      if (is_leaf_bc_node_not_subset_of_bc_nodes_in_partition)
        {
          /**
           * Here the desired block cluster node in the leaf set is found. Then
