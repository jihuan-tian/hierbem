 ...
 * dimension as the specified space dimension. Therefore, the bounding box for a
 * 2D surface in 3D space is still a 3D box.
 */
...
private:
  ...
  /**
   * Two corner points of the bounding box. The point in the pair is the bottom
   * corner, i.e. it has a smaller component coordinate in each dimension;
   * while the second point is the top corner.
   */
  ...
  const DoFHandler<dim, spacedim> &dof_handler)
{
  ...
  // all the DoFs in the DoFHandler.
