 ...
 * @author Jihuan Tian
 */
#ifndef HIERBEM_INCLUDE_HMATRIX_HMATRIX_SUPPORT_H_
#define HIERBEM_INCLUDE_HMATRIX_HMATRIX_SUPPORT_H_
...
namespace HMatrixSupport
{
   ...
   * member functions of this class) change the content of the \hmatrix in
   * some ways. For example, they may invert the matrix, or may replace it by
   * a matrix whose columns represent the eigenvectors of the original content
   * of the matrix. The elements of this enumeration are therefore used to
   * track what is currently being stored by this object.
   *
   ...
   * with memory.The matrix blocks in the lower triangular part are zero
   * valued. Their basic matrix information, such as dimension, is still
   * maintained but the memory for the data is not allocated.
   ...
   *
   * \mynote{N.B. Here the diagonal/upper-triangular/lower-triangular block
   * does not mean the matrix block itself is a
   * diagonal/upper-triangular/lower-triangular matrix, but means that the
   * matrix block belongs to the diagonal/upper-triangular/lower-triangular
  ...
  /**
   * Determine the block types of the four submatrices from the given block
   * type of their parent \hmatrix.
   */
  void
...
#endif /* HIERBEM_INCLUDE_HMATRIX_HMATRIX_SUPPORT_H_ */
