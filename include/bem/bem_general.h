  ...
  MatrixType &target_full_matrix)
{
  ...
  const bool is_symmetric =
    target_full_matrix.get_property() == LAPACKSupport::Property::symmetric;
  ...
  for (unsigned int i = 0; i < kx_dofs_per_cell; i++)
    {
      for (unsigned int j = 0; j < ky_dofs_per_cell; j++)
        {
          if (!is_symmetric ||
              (is_symmetric && data.kx_local_dof_indices_permuted[i] >=
  ...
  MatrixType                      &target_full_matrix)
{
  ...
  const bool is_symmetric =
    kernel.is_symmetric() &&
    (&dof_handler_for_test_space == &dof_handler_for_trial_space);
  target_full_matrix.set_property(is_symmetric ?
  ...
  bem_values.compute_bilinear_form_cell_values_for_regular(
    dof_handler_for_trial_space.get_triangulation(),
    mapping_indices,
    normal_detector);
  ...
  PairCellWiseScratchDataForFullMatrix<dim, spacedim, KernelNumberType>
    scratch_data(dof_handler_for_test_space.get_fe(),
                 dof_handler_for_trial_space.get_fe(),
                 mappings,
...
/**
 * Assemble a BEM full matrix without TBB parallelization.
 *
