 ...
 * @author Jihuan Tian
 */
#ifndef HIERBEM_INCLUDE_CAD_MESH_SUBDOMAIN_TOPOLOGY_H_
#define HIERBEM_INCLUDE_CAD_MESH_SUBDOMAIN_TOPOLOGY_H_
...
SubdomainTopology<dim, spacedim>::print(std::ostream &out) const
{
  ...
  for (const auto &record : surface_to_subdomain)
    {
      out << record.first << ": " << record.second[0] << " " << record.second[1]
          << "\n";
    }
...
#endif // HIERBEM_INCLUDE_CAD_MESH_SUBDOMAIN_TOPOLOGY_H_
