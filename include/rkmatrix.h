...
namespace IdeoBEM
{
    ...
    , n(sigma_index_range[1] - sigma_index_range[0])
  {
    ...
    Assert(is_subset(sigma_index_range, M_sigma_index_range),
           ExcInternalError());
  ...
  template <typename Number>
  bool
  RkMatrix<Number>::convertToFullMatrix(
    LAPACKFullMatrixExt<Number> &matrix) const
  {
  ...
  RkMatrix<Number>::restrictToFullMatrix(
    ...
    const std::array<types::global_dof_index, 2> &M_sigma_index_range) const
  {
    Assert(is_subset(M_tau_index_range, rk_tau_index_range),
           ExcInternalError());
