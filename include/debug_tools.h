 ...
 * \author Jihuan Tian
 */
#ifndef HIERBEM_INCLUDE_DEBUG_TOOLS_H_
#define HIERBEM_INCLUDE_DEBUG_TOOLS_H_
...
void
print_mesh_info(std::ostream                       &out,
                const Triangulation<dim, spacedim> &triangulation)
{
  out << "Manifold dimension: " << dim << "\n"
      << "Space dimension: " << spacedim << "\n"
      << "No. of cells: " << triangulation.n_cells() << "\n"
      << "No. of active cells: " << triangulation.n_active_cells() << "\n"
      << "No. of faces: " << triangulation.n_faces() << "\n"
      << "No. of active faces: " << triangulation.n_active_faces() << "\n"
      << "No. of vertices: " << triangulation.n_vertices() << std::endl;
  ...
  //!        boundary_count)
  //!     {
  //!       out << pair.first << "(" << pair.second << " cells) ";
  //!     }
  //!   out << std::endl;
  //! }
}
 ...
 * \ rgb "red" \endcode
 *
 * @param fe_system The given FiniteElement object will be checked if it has
 * support points.
 * @param mapping
 * @param dof_handler
 * @param base_name
 ...
 * splot "./data_file.gpl" using 1:2:3:4 with labels offset 1,1 point pt 1 lc
 * \ rgb "red" \endcode
 * @param fe_system The given FESystem object will be checked if it has
 * support points.
 * @param mapping
 * @param dof_handler
 * @param base_name
      ...
      for (unsigned int i = 0; i < dim; i++)
        {
          if (i == dim - 1)
            ...
            {
              out << quad_cpu.point(q)(i) << separator;
            }
        }
 ...
 *
 * @param out
 * @param points The first dimension is the quadrature point number. The
 * second dimension is the coordinate component. The index for the second
 * dimension runs faster.
 * @param weights
 */
...
/**
 * Print the vector of @p Point objects as a @p n*dim matrix in Octave format.
 *
 * @tparam dim
  ...
  for (const auto &p : values)
    {
      for (unsigned int d = 0; d < dim - 1; d++)
        {
          out << std::setprecision(precision) << std::setw(width) << p[d]
              << " ";
        }

      out << std::setprecision(precision) << std::setw(width) << p[dim - 1]
          << "\n";
    }
...
#endif // HIERBEM_INCLUDE_DEBUG_TOOLS_H_
