...
public:
   ...
   * index sets in the two clusters. If their sizes are equal, then
   * 1. if the size is larger than 0, which means the index set is not empty and
   * the index range is still not generated, then compare the contents in the
   * index set.
   ...
   * range is set, the original index set will be immediately cleared for saving
   * memory.
   *
   * @param lower_bound
   * @param upper_bound
   ...
   * Calculate the diameter of the cluster. There is no cell size correction.
   *
   * \mynote{In this version, the index set held by the cluster is empty and the
   * index range takes effect.}
   *
 ...
 *
 * \mynote{The index sets inferred from the index ranges held by the two
 * clusters share a same internal DoF numbering, which need to be mapped to the
 * external numbering for accessing the list of support point coordinates.}
 *
 * @param cluster1
 * @param cluster2
 * @param internal_to_external_dof_numbering
 ...
 * \mynote{The index sets inferred from the index ranges held by the two
 * clusters share a same internal DoF numbering, which need to be mapped to the
 * external numbering for accessing the list of support point coordinates as
 * well as estimated cell sizes.}
 *
 * @param cluster1
 * @param cluster2
 * @param internal_to_external_dof_numbering
...
Cluster<spacedim, Number>::is_subset(const Cluster &cluster) const
{
  if (this->index_set.size() > 0 && cluster.index_set.size() > 0)
    ...
    {
      return ::is_subset(this->index_range, cluster.index_range);
    }
}
...
Cluster<spacedim, Number>::is_proper_superset(const Cluster &cluster) const
{
  if (this->index_set.size() > 0 && cluster.index_set.size() > 0)
    ...
    {
      return ::is_proper_superset(this->index_range, cluster.index_range);
    }
}
