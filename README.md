...
* Compared with the **finite element method (FEM)**, the Galerkin BEM offers several advantages:
  ...
  * **Open-domain problems**
    ...
    BEM naturally handles unbounded domains, which are common in electromagnetics and acoustics.

  * **Surface-only discretization**

    Only the boundary surface requires meshing, avoiding the complexity of generating full 3D meshes. This is particularly attractive for models with fine structures or extreme size ratios, where meshing may become the bottleneck — or even fail entirely.
